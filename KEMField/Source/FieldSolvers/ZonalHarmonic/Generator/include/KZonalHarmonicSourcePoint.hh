...
class KZonalHarmonicSourcePoint
{
  public:
    ...
    {
        return "ZonalHarmonicSourcePoint";
    }

    void SetValues(double z0, double rho, std::vector<double>& coeffs);

    virtual ~KZonalHarmonicSourcePoint()
    {
        fCoeffVec.clear();
    }

    void SetZ0(const double& d)
    {
        fZ0 = d;
        fFloatZ0 = (float) d;
    }
    void SetRho(const double& d)
    {
        fRho = d;
        fRhosquared = (float) fRho * (float) fRho;
        f1overRhosquared = 1. / fRhosquared;
    }

    int GetNCoeffs() const
    {
        return (int) fCoeffVec.size();
    }
    double GetZ0() const
    {
        return fZ0;
    }
    float GetFloatZ0() const
    {
        return fZ0;
    }
    double GetRho() const
    ...
    const double* GetRawPointerToCoeff() const
    {
        return &(fCoeffVec[0]);
    };
  ...
  public:
    template<typename Stream> friend Stream& operator>>(Stream& s, KZonalHarmonicSourcePoint& sp)
    {
    ...
    template<typename Stream> friend Stream& operator<<(Stream& s, const KZonalHarmonicSourcePoint& sp)
    {
