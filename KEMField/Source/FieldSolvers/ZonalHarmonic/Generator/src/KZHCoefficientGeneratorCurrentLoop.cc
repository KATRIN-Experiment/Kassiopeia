...
/**
   ...
   */
void KZHCoefficientGenerator<KCurrentLoop>::ComputeCentralCoefficients(double z0, double rho,
                                                                       std::vector<double>& coeff) const
{
    double rho_ring =
        sqrt((z0 - fCurrentLoop->GetZ()) * (z0 - fCurrentLoop->GetZ()) + fCurrentLoop->GetR() * fCurrentLoop->GetR());
...
/**
   ...
   */
void KZHCoefficientGenerator<KCurrentLoop>::ComputeRemoteCoefficients(double z0, double rho,
                                                                      std::vector<double>& coeff) const
{
    double rho_ring =
        sqrt((z0 - fCurrentLoop->GetZ()) * (z0 - fCurrentLoop->GetZ()) + fCurrentLoop->GetR() * fCurrentLoop->GetR());
