...
namespace KEMField
{
   ...
   */
  void KZHCoefficientGenerator<KCoil>::ComputeCentralCoefficients(double z0,
								  double rho,
								  std::vector<double>& coeff) const
  {
    ...
    std::vector< std::vector<double> > bhat(nCoeffs+1,
  					      std::vector<double>(31,0));
    ...
    for (int m=0;m<=M;m++)
    {
      ...
      Z=fCoil->GetZ1(); z=Z-z0;
      ro=sqrt(R*R+z*z);
      bhat[0][m] = KEMConstants::Mu0*sigmac/2.*z/ro;
      Z=fCoil->GetZ0(); z=Z-z0;
      ro=sqrt(R*R+z*z);
      bhat[0][m]-=KEMConstants::Mu0*sigmac/2.*z/ro;
   ...
   */
  void KZHCoefficientGenerator<KCoil>::ComputeRemoteCoefficients(double z0,
							double rho,
							std::vector<double>& coeff) const
  {
    int nCoeffs = coeff.size();
    ...
    std::vector< std::vector<double> > bs(nCoeffs+2,
  					    std::vector<double>(2,0));
    std::vector< std::vector<double> > bshat(nCoeffs+1,
  					       std::vector<double>(1001,0));
  ...
  {
    zMin = fCoil->GetZ0(); zMax = fCoil->GetZ1();
  }
}
