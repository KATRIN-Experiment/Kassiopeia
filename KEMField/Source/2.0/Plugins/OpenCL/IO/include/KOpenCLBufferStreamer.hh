...
*/
  ...
  struct KOpenCLBufferStreamerType
  {
  ...
  class KOpenCLBufferPolicyStreamer : public KOpenCLBufferStreamer
  {
  public:
    KOpenCLBufferPolicyStreamer() : KOpenCLBufferStreamer(), fWrite(true), fSurfacePolicy(NULL), fBuffer(NULL), fBufferSize(0), fCounter(0) {}
	...
	*this >> *policy;
    ...
    void SetBuffer(CL_TYPE* buffer) { fBuffer = buffer; }
    void SetBufferSize(unsigned int i) { fBufferSize = i; }

  protected:
    void AppendToBuffer(CL_TYPE x) { fBuffer[fCounter++] = x; }
    void ExtractFromBuffer(CL_TYPE& x) { x = fBuffer[fCounter++]; }
    virtual KOpenCLBufferStreamer& Self() { return *this; }
