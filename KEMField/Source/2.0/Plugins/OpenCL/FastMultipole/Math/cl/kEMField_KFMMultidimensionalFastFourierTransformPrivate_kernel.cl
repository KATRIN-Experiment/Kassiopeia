...
#include "kEMField_KFMFastFourierTransformUtilitiesPrivate.cl"
...
MultidimensionalFastFourierTransform_Stage(unsigned int S, //number of transforms
                                           ...
                                           __global CL_TYPE2* restrict data)
#else
MultidimensionalFastFourierTransform_Stage(unsigned int S, //number of transforms
                                           unsigned int D, //d = 0, 1, ...FFT_NDIM-1 specifies the dimension being transformed
                                           int isForward, //1 = forward FFT, 0 = backward FFT
                                           const __global const unsigned int* restrict array_dimensions,
                                           const __global CL_TYPE2* restrict twiddle, //fft twiddle factors
                                           const __global CL_TYPE2* restrict conj_twiddle, //conjugate fft twiddle factors
                                           const __global CL_TYPE2* restrict scale, //bluestein scale factors
                                           const __global CL_TYPE2* restrict circulant, //bluestein circulant vector
                                           const __global const unsigned int* permutation_array, //bit reversal permutation indices
                                           __global CL_TYPE2* restrict data)
#endif
{
    ...
    //private memory workspace to perform the selected 1d fft
    __private CL_TYPE2 buffer[FFT_BUFFERSIZE];
