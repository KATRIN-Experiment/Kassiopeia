...
//we used stride memory access in order to reduce the number of bank conflicts
//in local memory which can degrade the speed quite substantially
inline unsigned int StridedIndex(unsigned int stride, unsigned int offset, unsigned int index)
{
...
//array permutation
void
PermuteArray(unsigned int N, unsigned int offset, unsigned int stride, __constant const unsigned int* permutation_index_arr, __local CL_TYPE2* arr)
{
    ...
    for(unsigned int i=0; i<N; i++)
    {
        ...
        {
            //swap values
            val = arr[StridedIndex(stride, offset, i)];
            arr[StridedIndex(stride, offset, i)] = arr[StridedIndex(stride, offset, perm)];
            arr[StridedIndex(stride, offset, perm)] = val;
        }
    }
}
...
void
FFTRadixTwo_DIT(unsigned int N, unsigned int offset, unsigned int stride, __local CL_TYPE2* data, __constant const CL_TYPE2* twiddle)
{
    //temporary workspace
    CL_TYPE2 H0;
    CL_TYPE2 H1;
    ...
    for(unsigned int stage = 0; stage < logN; stage++)
    {
        ...
        for(unsigned int n = 0; n < n_butterfly_groups; n++)
        {
            ...
            for(unsigned int k=0; k < butterfly_width; k++)
            {
                ...
                H0 = data[StridedIndex(stride, offset, butterfly_index)];
                H1 = data[StridedIndex(stride, offset, butterfly_index + butterfly_width)];
                ...
                data[StridedIndex(stride, offset, butterfly_index)] = H0;
                data[StridedIndex(stride, offset, butterfly_index + butterfly_width)] = H1;
            }
        }
    }
}
...
void
FFTRadixTwo_DIF(unsigned int N, unsigned int offset, unsigned int stride, __local CL_TYPE2* data, __constant const CL_TYPE2* twiddle)
{
    //temporary workspace
    CL_TYPE2 H0;
    CL_TYPE2 H1;
    ...
    for(unsigned int stage = 0; stage < logN; stage++)
    {
        ...
        for(unsigned int n = 0; n < n_butterfly_groups; n++)
        {
            ...
            for(unsigned int k=0; k < butterfly_width; k++)
            {
                ...
                data[StridedIndex(stride, offset, butterfly_index)] = H0;
                data[StridedIndex(stride, offset, butterfly_index + butterfly_width)] = Z;
            }
        }
    }
}
...
FFTBluestein(unsigned int N,
             ...
             __local CL_TYPE2* data) //must be size M, initially filled up to N
{
    ...
    {
        A = data[StridedIndex(stride, offset, i)];
        B = scale[i];
        Z.s0 = (A.s0)*(B.s0) - (A.s1)*(B.s1);
        Z.s1 =  (A.s1)*(B.s0) + (A.s0)*(B.s1);
        data[StridedIndex(stride, offset, i)] = Z;
    }
    ...
    FFTRadixTwo_DIF(M, offset, stride, data, twiddle);
    ...
    FFTRadixTwo_DIT(M, offset, stride, data, conj_twiddle);
    ...
    for(size_t i=0; i<N; i++)
    {
        ...
        Z.s0 = (A.s0)*(B.s0) - (A.s1)*(B.s1);
        Z.s1 = (A.s0)*(B.s1) + (A.s1)*(B.s0);
        data[StridedIndex(stride, offset, i)] = norm*Z;
    }

}

#endif /* KFMFastFourierUtilitiesLocal_Defined_H */
