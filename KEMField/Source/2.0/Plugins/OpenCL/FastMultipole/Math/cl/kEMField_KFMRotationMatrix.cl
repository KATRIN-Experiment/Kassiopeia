...
CL_TYPE4
EulerAnglesFromMatrix(CL_TYPE R[3][3]) //R is ptr to 3x3 array
{
    CL_TYPE4 ret_val;
    ...
    {
        beta = acos( R[2][2] );
        alpha = atan2( (-1.0*R[2][1] )/sin(beta), (R[2][0])/sin(beta) );
        gamma = atan2( (-1.0*R[1][2] )/sin(beta), -1.0*(R[0][2])/sin(beta)  ) ;
    }
    else
    {
        if( fabs(1.0 - R[2][2])  <= tol)
        {
            alpha =  atan2(R[1][0], R[0][0]);
            beta = 0.0;
            gamma = 0.0;
        }
        else if( fabs(1.0 + R[2][2]) <= tol)
        {
...
MatrixFromAxisAngle(CL_TYPE cos_angle, CL_TYPE sin_angle, CL_TYPE4 axis) //R is ptr to 3x3 array
{
    ...
    R[0][0] = axis.s0*axis.s0;
    R[0][1] = axis.s0*axis.s1;
    R[0][2] = axis.s0*axis.s2;
    R[1][0] = axis.s1*axis.s0;
    R[1][1] = axis.s1*axis.s1;
    R[1][2] = axis.s1*axis.s2;
    R[2][0] = axis.s2*axis.s0;
    R[2][1] = axis.s2*axis.s1;
    R[2][2] = axis.s2*axis.s2;
    ...
    R[0][1] -= sin_angle*(axis.s2);
    R[0][2] += sin_angle*(axis.s1);
    R[1][2] -= sin_angle*(axis.s0);

    R[1][0] += sin_angle*(axis.s2);
    R[2][0] -= sin_angle*(axis.s1);
    R[2][1] += sin_angle*(axis.s0);
