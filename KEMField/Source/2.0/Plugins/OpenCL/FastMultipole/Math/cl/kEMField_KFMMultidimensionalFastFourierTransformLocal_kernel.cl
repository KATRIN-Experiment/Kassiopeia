#ifndef KFMMultidimensionalFastFourierTransformLocal_Defined_H
#define KFMMultidimensionalFastFourierTransformLocal_Defined_H

#include "kEMField_defines.h"
#include "kEMField_KFMArrayMath.cl"
#include "kEMField_KFMFastFourierTransformUtilitiesLocal.cl"
...
__kernel void
MultidimensionalFastFourierTransformLocal_Stage(unsigned int S, //number of transforms
                                           ...
                                           __local CL_TYPE2* workspace)
{
    ...
    __local CL_TYPE2* buffer = workspace;
    //replace the above line with the following in order to demonstrate the existence of memory bank conflicts
    //__local CL_TYPE2* buffer = &(workspace[i_local*FFT_BUFFERSIZE]); i_local = 0; group_size = 1;
    ...
    if(i_global < S*n_local_1d_transforms) //thread id must be less than total number of 1d fft's
    {
        ...
        {
            buffer[StridedIndex(group_size, i_local, i)] = data_proxy[minor_offset + i*minor_stride];
        }
        ...
        if(isForward == 0)
        {
            ...
            for(unsigned int i=0; i<dim[D]; i++)
            {
                buffer[StridedIndex(group_size, i_local, i)].s1 *= -1.0;
            }
        }
        ...
        if( IsPowerOfBase(dim[D], 2) )
        {
            ...
            PermuteArray(dim[D], i_local, group_size, permutation_array, buffer);
            FFTRadixTwo_DIT(dim[D], i_local, group_size, buffer, twiddle);
        }
        else
        {
            //not a power of 2 length
            //execute FFT using the bluestein algorithm for an arbitrary length array
            FFTBluestein(dim[D], BluesteinArraySize(dim[D]), i_local, group_size , twiddle, conj_twiddle, scale, circulant, buffer);
        }
        ...
        for(unsigned int i=0; i<dim[D]; i++)
        {
            data_proxy[minor_offset + i*minor_stride] = buffer[StridedIndex(group_size, i_local, i)];
        }
...
#endif /* KFMMultidimensionalFastFourierTransformLocal_Defined_H */
