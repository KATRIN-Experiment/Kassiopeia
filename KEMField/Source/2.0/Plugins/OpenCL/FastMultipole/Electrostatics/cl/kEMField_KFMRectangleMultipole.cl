                         ...
                         CL_TYPE2* mom_output)
{
    ...
    ComputeTriangleMomentAnalyticTerms(max_degree, equatorial_plm, 2.0*basis.s6, basis.s0, basis.s1, basis.s2, mom_output);
    ...
    ComputeTriangleMomentAnalyticTerms(max_degree, equatorial_plm, 2.0*basis.s6, basis.s0, basis.s1, basis.s2, mom_scratch);

    //rotate the moments of triangle B by 90 degrees
    ApplySingleRotation_Z(max_degree, M_PI/2.0, mom_scratch, scratch1, scratch2);

    //now we add the two triangle's moments together
    for(int i=0; i<size; i++)
    {
        //add the moments of both triangles together
        mom_output[i] += mom_scratch[i];
        //copy output back into scratch
        mom_scratch[i] = mom_output[i];
    }

    //compute the moments of the mirror image by rotation
    ApplySingleRotation_Z(max_degree, M_PI, mom_scratch, scratch1, scratch2);
    ...
    ApplyFullEulerRotation_ZYZ(max_degree, pinchon_j, alpha, beta, gamma, mom_output, scratch1, scratch2);
