...
namespace KEMField
{
    ...
    void
    KFMVTKElectrostaticTreeViewer::ApplyAction(KFMElectrostaticNode* node)
    {
        if(node != NULL && !( node->HasChildren()) ) //we only visit leaf nodes
        {
            ...
            do
            {
                if(temp_node != NULL)
                {
                    KFMIdentitySetList* id_set_list = KFMObjectRetriever<KFMElectrostaticNodeObjects, KFMIdentitySetList >::GetNodeObject(temp_node);
                    if(id_set_list != NULL)
                    {
            ...
            double field[3]; fFieldCalc.ElectricField(cube->GetCenter(),field);
    ...
    void KFMVTKElectrostaticTreeViewer::GenerateGeometryFile(std::string fileName)
    {
        ...
        std::string ugname = "FastMultipoleTreeData.vtu";
        ...
        ugwriter->SetFileName(ugname.c_str());
        #if VTK_MAJOR_VERSION <= 5
        ugwriter->SetInput(fGrid);
        #else
        ugwriter->SetInputData(fGrid);
        #endif
        ugwriter->Write();
    }
