...
class KFMBox
{
    public:
        ...
        void SetLength(const double* len){for(unsigned int i=0; i<NDIM; i++){fLength[i] = len[i];} };
        void SetCenter(const double* center){ for(unsigned int i=0; i<NDIM; i++){fCenter[i] = center[i];} }

        //geometric property retrieval
        KFMPoint<NDIM> GetCenter() const {return KFMPoint<NDIM>(fCenter);};
        double GetLength(unsigned int i) const {return fLength[i];};
...
template<unsigned int NDIM>
inline double& KFMBox<NDIM>::operator[](unsigned int i)
{
    if(i < NDIM)
    {
        return fCenter[i];
...
template<unsigned int NDIM>
inline const double& KFMBox<NDIM>::operator[](unsigned int i) const
{
    if(i < NDIM)
    {
        return fCenter[i];
