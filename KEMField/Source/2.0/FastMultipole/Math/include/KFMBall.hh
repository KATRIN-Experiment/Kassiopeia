...
class KFMBall
{
    public:
        ...
        KFMBall()
        {
            for(unsigned int i=0; i<NDIM+1; i++){ fData[i] = 0.0; }
        };
        ...
        void SetCenter(KFMPoint<NDIM> center){for(unsigned int i=0; i<NDIM; i++){fData[i] = center[i]; } };
...
template<unsigned int NDIM>
inline double& KFMBall<NDIM>::operator[](unsigned int i)
{
    return fData[i];
}

template<unsigned int NDIM>
inline const double& KFMBall<NDIM>::operator[](unsigned int i) const
{
    return fData[i];
}
