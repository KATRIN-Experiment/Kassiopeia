...
//    robinHood.Solve(A,x,b);
    ...
    //effecting the sparse matrix elements, because it is irrelevant for that purpose
    params.degree = 6;
...
//    }
    ...
    #ifdef KEMFIELD_USE_OPENCL
    fast_solver = new KFMElectrostaticFastMultipoleFieldSolver_OpenCL(ocl_container, *e_tree);
    #else
    fast_solver = new KFMElectrostaticFastMultipoleFieldSolver(surfaceContainer, *e_tree);
    #endif
//    fast_solver = new KFMElectrostaticFastMultipoleFieldSolver(surfaceContainer, *e_tree);

    std::cout<<"starting field eval"<<std::endl;
...
//            KEMThreeVector field = direct_solver->ElectricField(&surface_list, position);
    ...
    TF2* p_fmm = new TF2("fmm_potential", FieldDifference, xlow, xhigh, ylow, yhigh, 0);
