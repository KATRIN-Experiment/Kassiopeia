...
int main(int argc, char* argv[])
{
    ...
    oclCalc->UseDefault();
    if(oclforce == 1){oclCalc->ForceAnalytic();};
    if(oclforce == 2){oclCalc->ForceNumerical();};
    oclCalc->Initialize();
    #endif
    ...
    //    double WireDiameter = 0.01;
        ...
        {
            std::cout<<"(analytic vs numerical) difference @ "<<i<<" = "<<std::abs(aMom[i] - nMom[i])<<std::endl;
        }
        ...
        for(unsigned int i=0; i<aMom.size(); i++)
        {
            std::cout<<"(analytic vs OpenCL)  difference @ "<<i<<" = "<<std::abs(aMom[i] - oclMom[i])<<std::endl;
        }
