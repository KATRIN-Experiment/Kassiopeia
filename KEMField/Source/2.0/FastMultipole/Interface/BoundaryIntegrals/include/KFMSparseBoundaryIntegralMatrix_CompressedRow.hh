...
class KFMSparseBoundaryIntegralMatrix_CompressedRow: public KBoundaryIntegralMatrix< FastMultipoleIntegrator, false >
{
    public:
        ...
        KFMSparseBoundaryIntegralMatrix_CompressedRow(KSurfaceContainer& c, FastMultipoleIntegrator& integrator, unsigned int verbosity=0):
        ...
        virtual void Multiply(const KVector<ValueType>& x, KVector<ValueType>& y) const
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    while(n_elements_to_buffer + fColumnIndexListPointers[row_stop_index]->size() < fBufferSize && row_stop_index < fDimension)
                    {
    ...
    protected:
        ...
        void ConstructElementNodeAssociation()
        {
            ...
            for(unsigned int i=0; i<n_elem; i++)
            {
                ...
                KFMPoint<NDIM> p;
                for(unsigned int x=0; x<NDIM; x++){p[x] = centroid[x];};
                ...
                {
                    fNodes[i] = NULL;
                    kfmout<<"KFMSparseBoundaryIntegralMatrix_CompressedRow::ConstructElementNodeAssociation: Error, element centroid not found in region."<<kfmendl;
                    kfmexit(1);
                }
            }
        }
        ...
        void ConstructSparseMatrixRepresentation()
        {
            ...
            for(unsigned int i=0; i<n_elem; i++)
            {
                ...
                KFMExternalIdentitySet* eid_set = KFMObjectRetriever<ObjectTypeList, KFMExternalIdentitySet>::GetNodeObject(fNodes[i]);
