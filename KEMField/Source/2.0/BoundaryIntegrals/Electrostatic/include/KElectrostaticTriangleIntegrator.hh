...
namespace KEMField
{
  class KElectrostaticTriangleIntegrator
  {
  public:
    typedef KTriangle Shape;
    typedef KElectrostaticBasis::ValueType ValueType;

    KElectrostaticTriangleIntegrator() {}
    ~KElectrostaticTriangleIntegrator() {}

    double Potential(const KTriangle* source,
		       const KPosition& P) const;

    KEMThreeVector ElectricField(const KTriangle* source,
			       const KPosition& P) const;

    double Potential(const KSymmetryGroup<KTriangle>* source,
		       const KPosition& P) const;

    KEMThreeVector ElectricField(const KSymmetryGroup<KTriangle>* source,
			       const KPosition& P) const;
