...
namespace KEMField
{
   ...
   * \f}
   */
  double KElectrostaticConicSectionIntegrator::Potential(const KConicSection* source,const KPosition& P) const
  {
    static const double ln4=1.386294361119890;

    static double (*f[1])(const double*,double*)
      = {&KElectrostaticRingIntegrator::PotentialFromChargedRing};
  ...
  KEMThreeVector KElectrostaticConicSectionIntegrator::ElectricField(const KConicSection* source,const KPosition& P) const
  {
    static const double ln4=1.386294361119890;

    static double (*f[2])(const double*,double*)
      = {&KElectrostaticRingIntegrator::EFieldRFromChargedRing,
	 &KElectrostaticRingIntegrator::EFieldZFromChargedRing};
  ...
  double KElectrostaticConicSectionIntegrator::Potential(const KSymmetryGroup<KConicSection>* source, const KPosition& P) const
  {
    double potential = 0.;
    for (KSymmetryGroup<KConicSection>::ShapeCIt it=source->begin();it!=source->end();++it)
      potential += Potential(*it,P);
    return potential;
  }

  KEMThreeVector KElectrostaticConicSectionIntegrator::ElectricField(const KSymmetryGroup<KConicSection>* source, const KPosition& P) const
  {
