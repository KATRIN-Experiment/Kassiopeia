...
namespace KEMField
{
  class KMagnetostaticBoundaryIntegrator :
    public KMagnetostaticLineSegmentIntegrator,
    public KMagnetostaticRingIntegrator
  {
  public:
  ...
  private:
    ...
    class ShapeVisitor :
      public KSelectiveVisitor<KShapeVisitor,KTYPELIST_2(KLineSegment,KRing)>
    {
    public:
      using KSelectiveVisitor<KShapeVisitor,
			      KTYPELIST_2(KLineSegment,KRing)>::Visit;

      ShapeVisitor(KMagnetostaticBoundaryIntegrator& integrator)
    ...
    class BasisVisitor :
      public KSelectiveVisitor<KBasisVisitor,KTYPELIST_1(KMagnetostaticBasis)>
    {
    public:
      using KSelectiveVisitor<KBasisVisitor,
			      KTYPELIST_1(KMagnetostaticBasis)>::Visit;

      BasisVisitor() : fBasisValue(NULL) {}

      void Visit(KMagnetostaticBasis&);

      void SetBasisIndex(unsigned int i) { fBasisIndex = i; }
  ...
  template <class SourceShape>
  void KMagnetostaticBoundaryIntegrator::ComputeBoundaryIntegral(SourceShape& source)
  {
