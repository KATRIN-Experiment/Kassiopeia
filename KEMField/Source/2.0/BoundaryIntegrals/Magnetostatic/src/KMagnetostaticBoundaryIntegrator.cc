...
namespace KEMField
{
  ...
  void KMagnetostaticBoundaryIntegrator::BoundaryVisitor::Visit(KNeumannBoundary& boundary)
  {
    // TO DO: get the right values for magnetostatics!
    fIsDirichlet = false;
    fPrefactor = ((1. + static_cast<NeumannBoundary&>(boundary).GetNormalBoundaryFlux(fBoundaryIndex))/(1. - static_cast<NeumannBoundary&>(boundary).GetNormalBoundaryFlux(fBoundaryIndex)));
    fBoundaryValue = 0.;
  }
  ...
  KMagnetostaticBasis::ValueType KMagnetostaticBoundaryIntegrator::BoundaryIntegral(KSurfacePrimitive* source,KSurfacePrimitive* target,unsigned int i)
  {
    fTarget = target;
  ...
  KMagnetostaticBasis::ValueType KMagnetostaticBoundaryIntegrator::BoundaryValue(KSurfacePrimitive* surface,unsigned int i)
  {
    fBoundaryVisitor.SetBoundaryIndex(i);
