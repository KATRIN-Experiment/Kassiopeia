...
namespace KEMField
{
  KThreeVector KMagnetostaticRingIntegrator::VectorPotential(const KRing* source,const KPosition& P) const
  {
    ...
    double S = sqrt((source->GetP()[0]+r)*(source->GetP()[0]+r) + (P[2]-source->GetP()[2])*(P[2]-source->GetP()[2]));
  ...
  KThreeVector KMagnetostaticRingIntegrator::MagneticField(const KRing* source,const KPosition& P) const
  {
    ...
    double D = sqrt((source->GetP()[0]-r)*(source->GetP()[0]-r) + (P[2]-source->GetP()[2])*(P[2]-source->GetP()[2]));
    ...
    if (r>1.e-12)
    {
      ...
      B_r = KEMConstants::Mu0OverPi*(P[2]-source->GetP()[2])*source->GetP()[0]/S*(2./(S*S)*ek_Elliptic + e_Elliptic/(D*D));
  ...
  KThreeVector KMagnetostaticRingIntegrator::VectorPotential(const KSymmetryGroup<KRing>* source, const KPosition& P) const
  {
    KThreeVector A;
