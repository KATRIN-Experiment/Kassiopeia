...
void
KVMSpaceRectangle::SetVertices(const double* point0, const double* point1, const double* point2, const double* point3)
{
    ...
    double max_dist = l1; opposite_corner_index = 1;
    if(l2 > max_dist){max_dist = l2; opposite_corner_index = 2;}
    if(l3 > max_dist){max_dist = l3; opposite_corner_index = 3;}
...
void
KVMSpaceRectangle::GetVertices(double* point0, double* point1, double* point2, double* point3) const
{
    for(unsigned int i = 0; i<3; i++)
    {
        point0[i] = fP[i];
