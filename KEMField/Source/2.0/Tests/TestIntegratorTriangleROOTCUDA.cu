...
int main()
{
	...
	for( unsigned int k=0; k<=kmax; k++ ) {
		...
		for( it=container->begin<KElectrostaticBasis>(); it!=container->end<KElectrostaticBasis>(); ++it ) {
			...
			valNum = std::make_pair(intCUNum.ElectricField(itTri->GetShape(),fP),intCUNum.Potential(itTri->GetShape(),fP));
#else
			valQuad = fQuadIntegrator.ElectricFieldAndPotential(itTri->GetShape(),fP);
			valNum = intCUNum.ElectricFieldAndPotential(itTri->GetShape(),fP);
#endif
