...
namespace KEMField
{
  ...
  double KSolenoidIntegrator::A_theta(const double *p, double *par)
  {
	...
	eta_2 = lolim;
      ...
      A_theta[i] = dz*p[0]/(S*3.)*
	(eta_2*ellipticCarlsonSymmetricRJ(0.,eta_1,1.,eta_2) -
	 ellipticCarlsonSymmetricRD(0.,eta_1,1.));
    }
  ...
  double KSolenoidIntegrator::B_r(const double *p, double *par)
  {
	...
	eta = lolim;
      ...
      Br[i] = p[0]/S*(ellipticCarlsonSymmetricRF(0.,eta,1.) -
		      2.*ellipticCarlsonSymmetricRD(0.,eta,1.)/3.);
    }
  ...
  double KSolenoidIntegrator::B_z(const double *p, double *par)
  {
	...
	(ellipticCarlsonSymmetricRF(0.,eta_1,1.) -
	 dr*(1.-eta_2)/(6.*p[0])*
	 ellipticCarlsonSymmetricRJ(0.,eta_1,1.,eta_2));
    }
    return (Bz[1]-Bz[0]);
  }
}
