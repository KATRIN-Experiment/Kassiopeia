...
template< typename ValueType >
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::Initialize()
{
...
template< typename ValueType >
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::ResetAndInitialize()
{
...
template< typename ValueType >
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::AugmentKrylovSubspace()
{
    ...
    unsigned int row;
    for(unsigned int n=0; n<fDim; n++){ fWSub[n] = 0;};
    ...
    for(row=0; row <= fJ; row++)
    {
        ...
        if( row%fNProcesses == fProcessID )
        {
            ...
            fH[fJ][row] = h;
            for(unsigned int n=0; n<fDim; n++){ fWSub[n] += h*fV[row][n]; };
        }
        else
        {
    ...
    for(unsigned int n=0; n<fDim; n++){ fW[n] -= fWSub[n]; };
...
template< typename ValueType >
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::UpdateSolution()
{
    if(fJ != 0)
    {
...
template< typename ValueType >
double
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::InnerProduct(const Vector& a, const Vector& b)
{
    double result = 0.;
...
template <typename ValueType>
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::SetResidualVector(const Vector& v)
{
...
template <typename ValueType>
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::GetResidualVector(Vector& v) const
{
...
template <typename ValueType>
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::SetState(const KPreconditionedGeneralizedMinimalResidualState<ValueType>& state)
{
...
template <typename ValueType>
void
KPreconditionedGeneralizedMinimalResidual_MPI<ValueType>::ReconstructState()
{
