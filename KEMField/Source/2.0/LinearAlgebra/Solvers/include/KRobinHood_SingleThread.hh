...
namespace KEMField
{
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::Initialize()
  {
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::FindResidual()
  {
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::FindResidualNorm(double& residualNorm)
  {
    residualNorm = fResidual.InfinityNorm()/fBInfinityNorm;
  }

  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::IdentifyLargestResidualElement()
  {
    fMaxResidualIndex = 0;
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::ComputeCorrection()
  {
    fCorrection = (fB(fMaxResidualIndex) - fB_iterative(fMaxResidualIndex))/fA(fMaxResidualIndex,fMaxResidualIndex);
  }

  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::UpdateSolutionApproximation()
  {
    fX[fMaxResidualIndex] += fCorrection;
  }

  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::UpdateVectorApproximation()
  {
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::SetResidualVector(const Vector& v)
  {
  ...
  template <typename ValueType>
  void KRobinHood_SingleThread<ValueType>::GetResidualVector(Vector& v) const
  {
    for (unsigned int i = 0;i<fResidual.Dimension();i++)
      v[i] = fResidual(i);
  }
