    ...
    for (int i = 0; i < Ncoil; i++) {
        ...
        if (G[i] == -1)  // starting a new symmetry group
        {
            ...
            for (int ic = i + 1; ic < Ncoil; ic++)  // ic is here global coil index
            {
                ...
                ZA = ZB = 0.;
                for (int k = 0; k < 3; k++) {
                    A[k] = coil[ic][1 + k] - uc[k] * L / 2.;
                    B[k] = coil[ic][1 + k] + uc[k] * L / 2.;  // coil ic endpoints
                    ZA += (A[k] - C[k]) * u[k];
                    ZB += (B[k] - C[k]) * u[k];
                }
                for (int k = 0; k < 3; k++) {
...
void MagfieldCoils::CoilGroupWrite()
{
    ...
    for (int i = 0; i < Ncoil; i++) {
        ...
        output << scientific << setw(26) << coil[i][0] << setw(26) << coil[i][1] << setw(26) << coil[i][2] << setw(26)
               << coil[i][3] << endl;
        output << scientific << setw(26) << coil[i][4] << setw(26) << coil[i][5] << setw(26) << coil[i][6] << endl;
        output << scientific << setw(26) << coil[i][7] << setw(26) << coil[i][8] << setw(26) << coil[i][9] << setw(26)
               << coil[i][10] << endl;
...
void MagfieldCoils::DynamicMemoryAllocations()
{
    ...
    C0 = new double[nmax + 1];
    C1 = new double[nmax + 1];
    c1 = new double[nmax + 1];
    c2 = new double[nmax + 1];
    c3 = new double[nmax + 1];
    c4 = new double[nmax + 1];
    c5 = new double[nmax + 1];
    c6 = new double[nmax + 1];
    c7 = new double[nmax + 1];
    c8 = new double[nmax + 1];
    c9 = new double[nmax + 1];
    c10 = new double[nmax + 1];
    c11 = new double[nmax + 1];
    c12 = new double[nmax + 1];
...
void MagfieldCoils::Magfield2EllipticCoil(int i, double z, double r, double& Bz, double& Br)
{
    ...
    Rmin = coil[i][8];
    Rmax = coil[i][9];    // coil inner and outer radii
...
//     roremG[g]: remote convergence radius for group g at the remote source point z0remG[g].
{
    ...
    for (int g = 0; g < Ng; g++) {
        ...
        output << scientific << setw(10) << g << setw(10) << Nc[g] << setw(26) << z0remG[g] << setw(26) << roremG[g]
               << endl;
...
//     roremG[g]: remote convergence radius for group g at the remote source point z0remG[g].
{
    ...
    for (int c = 0; c < Nc[g]; c++)  // c: local coil index in group
    {
        ...
        double L = coil[i][7];
        double zA = Z[g][c] - L / 2.;
        double zB = Z[g][c] + L / 2.;  // coil edges
        if (zA < zmin)
            zmin = zA;
        if (zB > zmax)
            zmax = zB;
...
//     i: coil index, n source constant index.
{
    ...
    for (int i = 0; i < Ncoil; i++) {
        // Coil i parameters:
        Rmin = coil[i][8];
        Rmax = coil[i][9];                             // inner and outer radius
        sigma = coil[i][10];                           // current density
...
bool MagfieldCoils::Magfield2Magcharge(int i, double z, double r, double& Bz, double& Br, double& rc)
{
    ...
    L = coil[i][7];
    Rmin = coil[i][8];
    Rmax = coil[i][9];    // coil length, inner and outer radius
    sigma = coil[i][10];  // current density
    Zmin = -L / 2.;
    Zmax = L / 2.;  // coil endpoints relative to coil center
...
bool MagfieldCoils::MagfieldCoil(int i, const double* P, double* B)
{
    ...
    {
        double L = coil[i][7];
        double Rmax = coil[i][9];  // coil length and outer radius
        double Zmin = -L / 2.;
...
bool MagfieldCoils::SourcePointSearching(bool coil, int ig, double z, double r, int type, int& jbest, double& rcbest)
{
    ...
    if (coil == true)  // coil case
    {
        ...
        {
            rcmin2 = rclimit * rclimit;
            jbest = -1;
            for (int j = 0; j < Nsp[i]; j++) {
                delz = z - z0cen[i][j];
                ro2 = r2 + delz * delz;
                rocen2 = rocen[i][j] * rocen[i][j];
        ...
        else  // only the 3 or the 11 source points near the last source point jlast are searched
        {
            ...
            if (type == 1)
                ...
                delj = 5;
            int jmin = jlast[i] - delj;
            if (jmin < 0)
                jmin = 0;
            int jmax = jlast[i] + delj;
            if (jmax > Nsp[i] - 1)
                jmax = Nsp[i] - 1;
            rcmin2 = rclimit * rclimit;
            jbest = -1;
    ...
    else   // group case
    {
        ...
        else  // only the 3 or the 11 source points near the last source point jlast are searched
        {
            ...
            if (type == 1)
                ...
                delj = 5;
            int jmin = jlastG[g] - delj;
            if (jmin < 0)
                jmin = 0;
            int jmax = jlastG[g] + delj;
            if (jmax > NspG[g] - 1)
                jmax = NspG[g] - 1;
            rcmin2 = rclimit * rclimit;
            jbest = -1;
