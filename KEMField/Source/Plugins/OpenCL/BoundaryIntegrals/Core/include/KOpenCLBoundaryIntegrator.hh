...
namespace KEMField
{
  ...
  class KOpenCLBoundaryIntegrator : public KOpenCLAction
  {
  public:
  ...
  protected:
    ...
    void StreamSourceToBuffer(const SourceShape* source) const;
  ...
  template <class BasisPolicy>
  KOpenCLBoundaryIntegrator<BasisPolicy>::KOpenCLBoundaryIntegrator(KOpenCLSurfaceContainer& c) : KOpenCLAction(c), fBufferP(NULL), fBufferShapeInfo(NULL), fBufferShapeData(NULL)
  {
  ...
  template <class BasisPolicy>
  KOpenCLBoundaryIntegrator<BasisPolicy>::~KOpenCLBoundaryIntegrator()
  {
    if (fBufferP) delete fBufferP;
    if (fBufferShapeInfo) delete fBufferShapeInfo;
    if (fBufferShapeData) delete fBufferShapeData;
  }

  template <class BasisPolicy>
  void KOpenCLBoundaryIntegrator<BasisPolicy>::AssignBuffers() const
  {
