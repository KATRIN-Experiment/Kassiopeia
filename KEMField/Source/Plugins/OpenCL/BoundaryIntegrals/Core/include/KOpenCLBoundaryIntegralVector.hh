...
class KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>> :
    ...
    public KOpenCLAction
{
  public:
    ...
    KBoundaryIntegralVector(KOpenCLSurfaceContainer& c, KOpenCLBoundaryIntegrator<BasisPolicy>& integrator);
...
template<class BasisPolicy> KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>>::~KBoundaryIntegralVector()
{
    if (fGetVectorElementKernel)
        delete fGetVectorElementKernel;
    if (fGetMaximumVectorElementKernel)
        delete fGetMaximumVectorElementKernel;
...
KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>>::operator()(unsigned int i) const
{
    cl_int i_[1];
    ...
    KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fGetVectorElementKernel,
                                                                     cl::NullRange,
                                                                     global,
                                                                     local);
...
template<class BasisPolicy>
const typename BasisPolicy::ValueType&
KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>>::InfinityNorm() const
{
    cl::NDRange global(1);
    cl::NDRange local(1);

    KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fGetMaximumVectorElementKernel,
                                                                     cl::NullRange,
                                                                     global,
                                                                     local);
...
void KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>>::ConstructOpenCLKernels() const
{
    ...
    // define fNLocal
    if (fNLocal == -1)
        fNLocal = fGetVectorElementKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(
...
template<class BasisPolicy> void KBoundaryIntegralVector<KOpenCLBoundaryIntegrator<BasisPolicy>>::AssignBuffers() const
{
