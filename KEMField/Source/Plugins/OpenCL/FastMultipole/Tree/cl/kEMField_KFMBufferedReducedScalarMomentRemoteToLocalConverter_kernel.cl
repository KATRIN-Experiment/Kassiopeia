                                                  ...
                                                  __global unsigned int* reversed_index)
{
    ...
    dim_size[0] = n_parent_nodes; //n parent nodes buffered in array
    dim_size[1] = (degree+1)*(degree+2)/2; //term stride
    dim_size[2] = spatial_stride;

    if(i_global < TotalArraySize(3, dim_size) )
    {
        ind[0] = 0; //node index
        ...
        //unreduced number of multipole terms
        unsigned int n_terms = (degree+1)*(degree+1);

        unsigned int tsi = ind[1]; //reduced multipole term index
        unsigned int spatial_offset = ind[2];
        unsigned int node_offset = ind[0]*dim_size[1]*dim_size[2];
        ...
        for(int n=0; n <= degree; n++)
        {
            ...
            for(int m=0; m <= n; m++)
            {
                ...
                source = remote_moments[node_offset + rssi*spatial_stride + spatial_offset];
                ...
                if(m > 0)
                {
                    ...
                    source = remote_moments[node_offset + rssi*spatial_stride + reversed_index[spatial_offset]];
        ...
        local_moments[node_offset + tsi*spatial_stride + spatial_offset] = target;
    }
}
