...
template<typename ObjectTypeList, typename SourceScalarMomentType, typename TargetScalarMomentType, typename KernelType,
         size_t SpatialNDIM>
class KFMScalarMomentRemoteToLocalConverter_OpenCL :
    public KFMScalarMomentRemoteToLocalConverter<ObjectTypeList, SourceScalarMomentType, TargetScalarMomentType,
                                                 KernelType, SpatialNDIM>
{
  public:
    typedef KFMScalarMomentRemoteToLocalConverter<ObjectTypeList, SourceScalarMomentType, TargetScalarMomentType,
                                                  KernelType, SpatialNDIM>
    ...
    virtual void ApplyAction(KFMNode<ObjectTypeList>* node)
    {
        if (node != NULL) {
            if (node->HasChildren() && this->ChildrenHaveNonZeroMoments(node)) {
                ...
                //now enqueue the kernel
                KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fAddKernel,
                                                                                 cl::NullRange,
                                                                                 global,
                                                                                 local);
#ifdef ENFORCE_CL_FINISH
                ...
                KOpenCLInterface::GetInstance()->GetQueue().enqueueReadBuffer(*fFFTDataBufferCL,
                                                                              CL_TRUE,
                                                                              0,
  ...
  protected:
    ...
    virtual void BuildBuffers()
    {
        ...
        if (m2l_size * sizeof(CL_TYPE2) > max_buffer_size) {
            ...
            if (m2l_size * sizeof(CL_TYPE2) > total_mem_size) {
                ...
                kfmout << "KFMScalarMomentRemoteToLocalConverter::BuildBuffers: Error. Cannot allocate buffer of size: "
                ...
                size_t n_buffers_needed =
                    std::ceil(((double) (m2l_size * sizeof(CL_TYPE2))) / ((double) max_buffer_size)) + 1;
                ...
                for (unsigned int i = 0; i < fNM2LBuffers; i++) {
                    if (i + 1 != fNM2LBuffers) {
                        ...
                        CL_ERROR_TRY
                        {
                            fM2LCoeffBufferCL[i] = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                                                  CL_MEM_READ_ONLY,
            ...
            CL_ERROR_TRY
            {
                fM2LCoeffBufferCL[0] = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                                      CL_MEM_READ_ONLY,
        ...
        if (this->fIsScaleInvariant) {
            ...
            for (size_t level = 0; level <= this->fMaxTreeDepth; level++) {
                ...
                if (level == 1) {
                    div_power = this->fTopLevelDivisions;
                };
        ...
        for (unsigned int i = 0; i < fNM2LBuffers; i++) {
            if (i + 1 != fNM2LBuffers) {
                ...
                std::complex<double>* ptr =
                    &(this->fPtrM2LCoeff[(n_terms_completed * (this->fNTerms)) * (this->fTotalSpatialSize)]);
                KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*(fM2LCoeffBufferCL[i]),
                                                                               CL_TRUE,
                                                                               0,
                                                                               elements_in_buffer * sizeof(CL_TYPE2),
                                                                               ptr);
    ...
    virtual void ConstructConvolutionKernel()
    {
        ...
        clFile << KOpenCLInterface::GetInstance()->GetKernelPath()
               << "/kEMField_KFMScalarMomentRemoteToLocalConverter_kernel.cl";
    ...
    virtual void ConstructAddKernel()
    {
        ...
        std::stringstream clFile;
        clFile << KOpenCLInterface::GetInstance()->GetKernelPath()
               << "/kEMField_KFMPointwiseComplexVectorAdd_kernel.cl";
        ...
        //get n-local
        fNAddLocal =
            fAddKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(KOpenCLInterface::GetInstance()->GetDevice());

        unsigned int preferredWorkgroupMultiple =
            fAddKernel->getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(
                KOpenCLInterface::GetInstance()->GetDevice());
    ...
    void EnqueueWriteOriginalLocalCoefficients()
    {
        ...
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fWorkspaceBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       moment_size * sizeof(CL_TYPE2),
                                                                       this->fPtrOrigLocalCoeff);
#ifdef ENFORCE_CL_FINISH
        KOpenCLInterface::GetInstance()->GetQueue().finish();
#endif
    }
    ...
    virtual void CheckDeviceProperites()
    {
        ...
        if (m2l_size * sizeof(CL_TYPE2) > total_mem_size) {
            ...
            kfmout
                << "KFMScalarMomentRemoteToLocalConverter_OpenCL::CheckDeviceProperites: Error. Cannot allocate buffer of size: "
                << size_to_alloc_mb << " MB on a device with max allowable buffer size of: " << max_size_mb
                << " MB and total device memory of: " << total_size_mb << " MB." << kfmendl;
            kfmexit(1);
        }
