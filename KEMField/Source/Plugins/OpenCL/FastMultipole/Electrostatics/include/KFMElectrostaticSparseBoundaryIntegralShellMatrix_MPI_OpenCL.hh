#ifndef KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL_HH__
#define KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL_HH__
...
/*
*
*@file KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL.hh
*@class KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL
...
class KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL :
    KSquareMatrix<typename FastMultipoleIntegrator::Basis::ValueType>
{
  public:
    ...
    KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL(KSurfaceContainer& c,
        ...
        fVerbosity(integrator.GetVerbosity())
    {
        ...
        ss << " -DKEMFIELD_INTEGRATORFILE_CL=<"
           << "kEMField_ElectrostaticNumericBoundaryIntegrals.cl"
           << ">";
        ...
        unsigned int min_workgroup_size = fNComputeLocal;
        if (fNReduceLocal < min_workgroup_size) {
            min_workgroup_size = fNReduceLocal;
        };
    ...
    virtual ~KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL()
    {
    ...
    virtual void Multiply(const KVector<ValueType>& x, KVector<ValueType>& y) const
    {
        ...
        for (size_t i = 0; i < fDimension; i++) {
            fInputVector[fOCLSurfaceContainer->GetSortedIndexFromNormalIndex(i)] = x(i);
            fOutputVector[i] = 0.0;
        }

        //copy the input vector x over to the GPU
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fInputVectorBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       fDimension * sizeof(CL_TYPE),
                                                                       &(fInputVector[0]));

        //zero out the output vector on the GPU
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fOutputVectorBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       fDimension * sizeof(CL_TYPE),
                                                                       &(fOutputVector[0]));
        ...
        KOpenCLInterface::GetInstance()->GetQueue().enqueueReadBuffer(*fOutputVectorBufferCL,
                                                                      CL_TRUE,
                                                                      0,
                                                                      fDimension * sizeof(CL_TYPE),
                                                                      &(fOutputVector[0]));
        ...
        //copy the output vector into the workspace for mpi reduction
        //resorted according the index map
        for (size_t i = 0; i < fDimension; i++) {
            fTempIn[fOCLSurfaceContainer->GetNormalIndexFromSortedIndex(i)] = fOutputVector[i];
        }
        ...
        //            for(size_t i=0; i<fDimension; i++)
        //            {
        //                norm += fTempIn[i]*fTempIn[i];
        //            }
        //            std::cout<<"Process # "<<KMPIInterface::GetInstance()->GetProcess()<<" sparse mat-vec norm = "<<std::sqrt(norm)<<std::endl;
  ...
  protected:
    void Initialize()
    {
        ...
        KFMDenseBlockSparseMatrixStructureGenerator<KFMElectrostaticNodeObjects, KSquareMatrix<ValueType>>
            dbsmGenerator;
    ...
    void BuildBlockComputeKernel()
    {
        ...
        fBlockComputeKernel =
            k_builder.BuildKernel(clFile.str(),
                                  std::string("ElectrostaticSparseShellMatrixVectorProduct_ComputeBlock"),
                                  fBuildFlags);

        //get n-local
        fNComputeLocal = fBlockComputeKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(
            KOpenCLInterface::GetInstance()->GetDevice());


        if (fVerbosity > 2) {
            kfmout
                << "KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL::BuildBlockComputeKernel: Number of local threads is: "
                << fNComputeLocal << kfmendl;
        }
    }
    ...
    void BuildBlockReduceKernel()
    {
        ...
        //get n-local
        fNReduceLocal = fBlockReduceKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(
            KOpenCLInterface::GetInstance()->GetDevice());

        if (fVerbosity > 2) {
            kfmout
                << "KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL::BuildBlockReduceKernel: Number of local threads is: "
                << fNReduceLocal << kfmendl;
        }
    }
    ...
    void BuildBuffers()
    {
        ...
        fRowSizeBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                          CL_MEM_READ_ONLY,
                                          max_blocks * sizeof(size_t));

        //create the column index buffer
        fColSizeBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                          CL_MEM_READ_ONLY,
                                          max_blocks * sizeof(size_t));
        ...
        fRowBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                      CL_MEM_READ_ONLY,
                                      max_row_buffer_size * sizeof(size_t));

        //create the column index buffer
        size_t max_col_buffer_size = fStructure.GetMaxNumberOfColumnIndicesInAnyBuffer();

        fColumnBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                         CL_MEM_READ_ONLY,
                                         max_col_buffer_size * sizeof(size_t));

        fBlockBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                        CL_MEM_READ_WRITE,
                                        fElementBufferSize * sizeof(CL_TYPE));

        //input and output vectors
        fInputVectorBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                              CL_MEM_READ_ONLY,
                                              fDimension * sizeof(CL_TYPE));

        fOutputVectorBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                               CL_MEM_READ_WRITE,
                                               fDimension * sizeof(CL_TYPE));
    }
    ...
    void BufferedMultiply() const
    {
        ...
        for (size_t buffer_id = 0; buffer_id < fStructure.GetNBuffers(); buffer_id++) {
            ...
            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fRowSizeBufferCL,
                                                                           CL_TRUE,
                                                                           0,
                                                                           n_blocks * sizeof(size_t),
                                                                           &(fRowSizeBuffer[0]));

            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fColSizeBufferCL,
                                                                           CL_TRUE,
                                                                           0,
                                                                           n_blocks * sizeof(size_t),
                                                                           &(fColSizeBuffer[0]));

            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fRowBufferCL,
                                                                           CL_TRUE,
                                                                           0,
                                                                           n_rows * sizeof(size_t),
                                                                           &(fRowIndices[row_start]));

            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fColumnBufferCL,
                                                                           CL_TRUE,
                                                                           0,
                                                                           n_cols * sizeof(size_t),
                                                                           &(fColumnIndices[col_start]));

            //now figure out the global number of items we need to run the block compute kernel on
            unsigned int nDummy = fNComputeLocal - (n_elem % fNComputeLocal);
            if (nDummy == fNComputeLocal) {
                nDummy = 0;
            };

            //run the kernel
            KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fBlockComputeKernel,
                                                                             cl::NullRange,
                                                                             cl::NDRange(n_elem + nDummy),
                                                                             cl::NDRange(fNComputeLocal),
                                                                             NULL,
                                                                             NULL);
            ...
            if (nDummy == fNReduceLocal) {
                nDummy = 0;
            };

            //run the kernel
            KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fBlockReduceKernel,
                                                                             cl::NullRange,
                                                                             cl::NDRange(n_blocks + nDummy),
                                                                             cl::NDRange(fNReduceLocal));
    ...
    mutable unsigned int fNComputeLocal;
    mutable unsigned int fNReduceLocal;
...
#endif /* KFMElectrostaticSparseBoundaryIntegralShellMatrix_MPI_OpenCL_H__ */
