...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::SetTree(KFMElectrostaticTree* tree)
{
    fTree = tree;
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::SetMultipoleNodeSet(KFMSpecialNodeSet<KFMElectrostaticNodeObjects>* multipole_node_set)
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::SetPrimaryNodeSet(KFMSpecialNodeSet<KFMElectrostaticNodeObjects>* local_node_set)
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ConstructCachedNodeIdentityLists()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::Finalize()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ReadOutLocalCoefficients()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::Initialize()
{
    //we determine the number of nodes to buffer from the maximum device memory buffer size
    //we only use 75% of max, just to be safe
    double buffer_size = 0.75*KOpenCLInterface::GetInstance()->GetDevice().getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>();
    ...
    fHelperArrayWrapper = new KFMArrayWrapper<std::complex<double>, KFMELECTROSTATICS_DIM + 1>( &(fRawHelperArray[0]), fHelperDimensionSize );
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ApplyAction(KFMNode<KFMElectrostaticNodeObjects>* node)
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::BufferNode(KFMNode<KFMElectrostaticNodeObjects>* node)
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ExecuteBufferedAction()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ClearBuffers()
{
    fNBufferedNodes = 0;
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::BuildBuffers()
{
    ...
    fACoeffBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY, fStride*sizeof(CL_TYPE2));
    ...
    CL_ERROR_TRY
    {
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fACoeffBufferCL, CL_TRUE, 0, fStride*sizeof(CL_TYPE2), &(a_coeff[0]));
        #ifdef ENFORCE_CL_FINISH
        KOpenCLInterface::GetInstance()->GetQueue().finish();
        #endif
    }
    ...
    CL_ERROR_TRY
    {
        ...
        = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY, fNMaxBufferedNodes*fNChildren*sizeof(int) );
    }
    CL_ERROR_CATCH
    ...
    CL_ERROR_TRY
    {
        ...
        = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY, fNMaxBufferedNodes*fNChildren*sizeof(unsigned int) );
    }
    CL_ERROR_CATCH
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::AssignBuffers()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ApplyCopyAndScaleKernel()
{
    unsigned int n_global;
    unsigned int nDummy;
    ...
    KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fMultipoleNodeIDListBufferCL, CL_TRUE, 0, fNBufferedNodes*fNChildren*sizeof(int), &(fMultipoleNodeIDBuffer[0]));
    #ifdef ENFORCE_CL_FINISH
    KOpenCLInterface::GetInstance()->GetQueue().finish();
    #endif

    //enqueue write the block set ids
    KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fMultipoleBlockSetIDListBufferCL, CL_TRUE, 0, fNBufferedNodes*fNChildren*sizeof(unsigned int), &(fMultipoleBlockSetIDBuffer[0]) );
    #ifdef ENFORCE_CL_FINISH
    KOpenCLInterface::GetInstance()->GetQueue().finish();
    #endif
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ApplyTransformationKernel()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ApplyReduceAndScaleKernel()
{
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::CheckDeviceProperites()
{
    ...
    if(m2l_size*sizeof(CL_TYPE2) > max_buffer_size)
    {
        ...
        kfmout<<"KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::CheckDeviceProperites: Error. Cannot allocate buffer of size: "<<size_to_alloc_mb<<" MB on a device with max allowable buffer size of: "<<max_size_mb<<" MB and total device memory of: "<<total_size_mb<<" MB."<<kfmendl;
        kfmexit(1);
    }
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ConstructCopyAndScaleKernel()
{

    //Get name of kernel source file
    std::stringstream clFile;
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath() << "/kEMField_KFMElectrostaticBufferedRemoteToLocalCopyAndScale_kernel.cl";

    KOpenCLKernelBuilder k_builder;
    fCopyAndScaleKernel = k_builder.BuildKernel(clFile.str(), std::string("ElectrostaticBufferedRemoteToLocalCopyAndScale") );
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ConstructTransformationKernel()
{
    //Get name of kernel source file
    std::stringstream clFile;
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath() << "/kEMField_KFMElectrostaticBatchedRemoteToLocalTransformation_kernel.cl";
    ...
    KOpenCLKernelBuilder k_builder;
    fTransformationKernel = k_builder.BuildKernel(clFile.str(), std::string("BatchedRemoteToLocalTransformation"), build_flags );
    ...
    if(fDefaultWorkSize < fNTransformationLocal){fNTransformationLocal = fDefaultWorkSize;};
}

////////////////////////////////////////////////////////////////////////////////

void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ConstructReduceAndScaleKernel()
{
    //Get name of kernel source file
    std::stringstream clFile;
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath() << "/kEMField_KFMElectrostaticBufferedRemoteToLocalReduceAndScale_kernel.cl";
    ...
    KOpenCLKernelBuilder k_builder;
    fReduceAndScaleKernel = k_builder.BuildKernel(clFile.str(), std::string("ElectrostaticBufferedRemoteToLocalReduceAndScale"), build_flags );
...
void
KFMElectrostaticBatchedRemoteToLocalConverter_OpenCL::ConstructZeroComplexArrayKernel()
{
    //Get name of kernel source file
    std::stringstream clFile;
    ...
    unsigned int preferredWorkgroupMultiple = fZeroComplexArrayKernel->getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(KOpenCLInterface::GetInstance()->GetDevice() );
