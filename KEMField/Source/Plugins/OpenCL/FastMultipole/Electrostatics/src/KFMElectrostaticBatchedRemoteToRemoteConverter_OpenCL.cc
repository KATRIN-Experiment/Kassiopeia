...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::SetParameters(KFMElectrostaticParameters params)
{
    //set parameters
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::SetTree(KFMElectrostaticTree* tree)
{
    fTree = tree;
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::SetMultipoleNodeSet(
...
////////////////////////////////////////////////////////////////////////
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::Initialize()
{
    ...
    fM2MCoeff =
        new KFMArrayWrapper<std::complex<double>, KFMELECTROSTATICS_DIM + 2>(&(fRawM2MCoeff[0]), fDimensionSize);
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::CopyMomentsToDevice()
{
...
#pragma GCC diagnostic pop
    ...
    for (unsigned int i = 0; i < fNMultipoleNodes; i++) {
        KFMNode<KFMElectrostaticNodeObjects>* node = fMultipoleNodes->GetNodeFromSpecializedID(i);
        KFMElectrostaticMultipoleSet* set =
...
#pragma GCC diagnostic pop
    ...
    KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fNodeMomentBufferCL,
                                                                   CL_TRUE,
                                                                   0,
                                                                   size * sizeof(CL_TYPE2),
                                                                   &(moments[0]));
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::RecieveMomentsFromDevice()
{
...
////////////////////////////////////////////////////////////////////////
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::ApplyAction(KFMNode<KFMElectrostaticNodeObjects>* node)
{
        ...
        if (special_id != -1) {
            ...
            if (node_level != fCachedNodeLevel) {
                ...
                if ((fNBufferedNodes + n_moment_sets < fNMaxBufferedNodes) &&
                    (fNBufferedParentNodes < fNMaxParentNodes)) {
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::BufferNode(KFMNode<KFMElectrostaticNodeObjects>* node)
{
    //check if this node is a member of the non-zero multipole node set
    int special_id = fMultipoleNodes->GetSpecializedIDFromOrdinaryID(node->GetID());
    //get the number of children which have non-zero multipole moments
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::ExecuteBufferedAction()
{
    if (fNBufferedNodes != 0 && fNBufferedParentNodes != 0) {
        ...
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fParentNodeOffsetBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       fNBufferedParentNodes * sizeof(unsigned int),
                                                                       &(fParentNodeOffsetBuffer[0]));
#ifdef ENFORCE_CL_FINISH
        KOpenCLInterface::GetInstance()->GetQueue().finish();
#endif

        //write out the parent nodes size of child data
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fNChildNodeBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       fNBufferedParentNodes * sizeof(unsigned int),
                                                                       &(fNChildBuffer[0]));
        ...
        KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fParentNodeIDBufferCL,
                                                                       CL_TRUE,
                                                                       0,
                                                                       fNBufferedParentNodes * sizeof(unsigned int),
                                                                       &(fParentNodeIDBuffer[0]));
#ifdef ENFORCE_CL_FINISH
        KOpenCLInterface::GetInstance()->GetQueue().finish();
#endif
        ...
        KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fReduceKernel,
                                                                         cl::NullRange,
                                                                         cl::NDRange(nGlobal + nDummy),
                                                                         cl::NDRange(nLocal));
#ifdef ENFORCE_CL_FINISH
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::ClearBuffers()
{
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::BuildBuffers()
{
    ...
    CL_ERROR_TRY
    {
        fM2MCoeffBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                           CL_MEM_READ_ONLY,
    ...
    CL_ERROR_TRY
    {
        fTransformedMomentBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                                    CL_MEM_READ_WRITE,
                                                    fNMaxBufferedNodes * fStride * sizeof(CL_TYPE2));
    }
    CL_ERROR_CATCH
    ...
    CL_ERROR_TRY
    {
        fParentNodeOffsetBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                                   CL_MEM_READ_ONLY,
                                                   fNMaxParentNodes * sizeof(unsigned int));
    }
    CL_ERROR_CATCH

    CL_ERROR_TRY
    {
        fNChildNodeBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                             CL_MEM_READ_ONLY,
                                             fNMaxParentNodes * sizeof(unsigned int));
    }
    CL_ERROR_CATCH
    ...
    CL_ERROR_TRY
    {
        fParentNodeIDBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                               CL_MEM_READ_ONLY,
                                               fNMaxParentNodes * sizeof(unsigned int));
    }
    CL_ERROR_CATCH
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::AssignBuffers()
{
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::ConstructTransformationKernel()
{
    //Get name of kernel source file
    std::stringstream clFile;
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath()
           << "/kEMField_KFMElectrostaticBatchedRemoteToRemoteTransformation_kernel.cl";
    ...
    fTransformationKernel =
        k_builder.BuildKernel(clFile.str(), std::string("ElectrostaticBatchedRemoteToRemoteTransform"), build_flags);
...
void KFMElectrostaticBatchedRemoteToRemoteConverter_OpenCL::ConstructReduceKernel()
{
    //Get name of kernel source file
    std::stringstream clFile;
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath()
           << "/kEMField_KFMElectrostaticBatchedRemoteToRemoteReduce_kernel.cl";
    ...
    KOpenCLKernelBuilder k_builder;
    fReduceKernel =
        k_builder.BuildKernel(clFile.str(), std::string("ElectrostaticBatchedRemoteToRemoteReduce"), build_flags);

    //get n-local
    fNReduceLocal =
        fReduceKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(KOpenCLInterface::GetInstance()->GetDevice());

    unsigned int preferredWorkgroupMultiple =
        fReduceKernel->getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(
            KOpenCLInterface::GetInstance()->GetDevice());
