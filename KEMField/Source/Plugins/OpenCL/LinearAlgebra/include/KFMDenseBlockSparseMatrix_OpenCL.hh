...
template<typename ValueType> class KFMDenseBlockSparseMatrix_OpenCL : public KSquareMatrix<ValueType>
{
  public:
    ...
    {
        return fSuggestedMatrixElementBufferSize;
    };
    size_t GetSuggestedIndexBufferSize() const
    {
        return fSuggestedMatrixIndexBufferSize;
    };
    size_t GetSuggestedMaximumRowWidth() const
    {
        return fNLocal;
    };
    ...
    void Initialize()
    {
        ...
        if (result) {
            ...
            //error, abort
            delete structure_node;
            kfmout << "KFMDenseBlockSparseMatrix_OpenCL::Initialize(): Error, structure file: " << fStructureFileName
        ...
        for (unsigned int i = 0; i < rowSizes->size(); i++) {
            fRowSizes->push_back((unsigned int) rowSizes->at(i));
        };
        for (unsigned int i = 0; i < columnSizes->size(); i++) {
            fColumnSizes->push_back((unsigned int) columnSizes->at(i));
        };
        for (unsigned int i = 0; i < rowSizes->size(); i++) {
            fNElements->push_back((unsigned int) nElements->at(i));
        };
        for (unsigned int i = 0; i < rowSizes->size(); i++) {
            fRowOffsets->push_back((unsigned int) rowOffsets->at(i));
        };
        for (unsigned int i = 0; i < rowSizes->size(); i++) {
            fColumnOffsets->push_back((unsigned int) columnOffsets->at(i));
        };
        for (unsigned int i = 0; i < rowSizes->size(); i++) {
            fElementOffsets->push_back((unsigned int) elementOffsets->at(i));
        };
        ...
        if (!row_exists) {
            ...
            kfmout << "KFMDenseBlockSparseMatrix_OpenCL::Initialize(): Error, row file corrupt or not present."
                   << kfmendl;
        ...
        if (!col_exists) {
            ...
            delete fColumnFileInterface;
            delete fElementFileInterface;
            kfmout << "KFMDenseBlockSparseMatrix_OpenCL::Initialize(): Error, column file corrupt or not present."
                   << kfmendl;
        ...
        if (!elem_exists) {
            ...
            delete fElementFileInterface;
            kfmout
                << "KFMDenseBlockSparseMatrix_OpenCL::Initialize(): Error, matrix element file corrupt or not present."
                << kfmendl;
        ...
        if (fNLocal < fMatrixStructure.GetMaxRowWidth()) {
            //error, abort
            kfmout << "KFMDenseBlockSparseMatrix_OpenCL::Initialize(): Error, maximum row width "
                   << fMatrixStructure.GetMaxRowWidth() << " is greater than number of local threads: " << fNLocal
                   << ". Please reduce the maximum allowable row width parameter. " << kfmendl;
            kfmexit(1);
        }
  ...
  protected:
    ...
    void ConstructKernel()
    {
        ...
        clFile << KOpenCLInterface::GetInstance()->GetKernelPath()
               << "/kEMField_DenseBlockSparseMatrixVectorProduct_kernel.cl";
        ...
        if (fVerbosity >= 2) {
            kfmout << "KFMDenseBlockSparseMatrix_OpenCL::ConstructKernel: Number of local threads is: " << fNLocal
                   << kfmendl;
        }
    }
    ...
    void BuildBuffers()
    {
        ...
        fRowBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                      CL_MEM_READ_ONLY,
                                      max_row_size * sizeof(unsigned int));

        //create the column index buffer
        fColumnBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                         CL_MEM_READ_ONLY,
                                         max_col_size * sizeof(unsigned int));

        //create the matrix element buffer
        fElementBufferCL = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
                                          CL_MEM_READ_ONLY,
                                          max_block_size * sizeof(CL_TYPE));
    ...
    void SingleBufferMultiply() const
    {
        ...
        for (size_t n = 0; n < n_blocks; n++) {
            ...
            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*fRowBufferCL,
                                                                           CL_TRUE,
                                                                           0,
                                                                           nrows * sizeof(unsigned int),
                                                                           &(fRowIndices[(*fRowOffsets)[block_id]]));
            ...
            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                *fColumnBufferCL,
                ...
                &(fColumnIndices[(*fColumnOffsets)[block_id]]));

            KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(
                *fElementBufferCL,
                CL_TRUE,
                0,
                n_elements * sizeof(CL_TYPE),
                &(fMatrixElements[(*fElementOffsets)[block_id]]));
            ...
            //run the kernel
            KOpenCLInterface::GetInstance()->GetQueue().enqueueNDRangeKernel(*fKernel,
                                                                             cl::NullRange,
                                                                             cl::NDRange(nGlobal),
                                                                             cl::NDRange(nLocal));
        }
