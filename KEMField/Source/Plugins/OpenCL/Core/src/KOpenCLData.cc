...
namespace KEMField
{
  KOpenCLData::KOpenCLData(): fNLocal(std::numeric_limits<unsigned int>::max()),
  ...
  void KOpenCLData::ConstructOpenCLObjects()
  {
    ...
    for (std::vector<const KOpenCLAction*>::iterator action = fAssociatedActions.begin();action!=fAssociatedActions.end();++action)
    {
      if ((*action)->Enabled())
  ...
  void KOpenCLData::SetMinimumWorkgroupSizeForKernels(unsigned int nLocal)
  {
    ...
    {
      if (fIsConstructed)
	KEMField::cout<<"Warning: Buffers have already been constructed with a workgroup size "<<fNLocal<<"."<<KEMField::endl;
