...
template<typename ValueType, unsigned int Dim> class KBoundaryValue
{
  protected:
  ...
  public:
    ...
    void SetBoundaryValue(unsigned int i, ValueType v)
    {
        if (i < Dim)
            fBoundaryValue[i] = v;
    }
    ValueType GetBoundaryValue(unsigned int i = 0) const
    {
        return (i < Dim ? fBoundaryValue[i] : 0.);
    }
...
template<typename ValueType, unsigned int Dim> class KNormalBoundaryFlux
{
  protected:
  ...
  public:
    ...
    {
        fNormalBoundaryFlux[0] = v;
    }
    void SetNormalBoundaryFlux(unsigned int i, ValueType v)
    {
        if (i < Dim)
            fNormalBoundaryFlux[i] = v;
    }
    ValueType GetNormalBoundaryFlux(unsigned int i = 0) const
    {
        return (i < Dim ? fNormalBoundaryFlux[i] : 0.);
    }
...
template<class BasisPolicy, class BoundaryCondition> class KBoundaryType;
    ...
    public KBoundaryValue<typename BasisPolicy::ValueType, BasisPolicy::Dimension>
{
  ...
  public:
    static std::string Name()
    {
        return KDirichletBoundary::Name();
    }
};
    ...
    public KNormalBoundaryFlux<typename BasisPolicy::ValueType, BasisPolicy::Dimension>
{
  ...
  public:
    static std::string Name()
    {
        return KCauchyBoundary::Name();
    }
};

template<class BasisPolicy> bool KBoundaryType<BasisPolicy, KCauchyBoundary>::SameBoundaryAs(const KBoundary* rhs) const
{
    ...
    public KNormalBoundaryFlux<typename BasisPolicy::ValueType, BasisPolicy::Dimension>
{
  ...
  public:
    static std::string Name()
    {
        return KRobinBoundary::Name();
    }
};

template<class BasisPolicy> bool KBoundaryType<BasisPolicy, KRobinBoundary>::SameBoundaryAs(const KBoundary* rhs) const
{
...
class KBoundaryType<BasisPolicy, KIsolatedBoundary> : public KBoundary, public KIsolatedBoundary
{
  ...
  public:
    ...
    unsigned int GetBoundaryIndex() const
    {
        return fBoundaryIndex;
    }
};
