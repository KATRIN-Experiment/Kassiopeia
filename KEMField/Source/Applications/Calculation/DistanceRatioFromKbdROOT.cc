...
namespace KEMField
{
class DistRatioVisitor : public KSelectiveVisitor<KShapeVisitor, KTYPELIST_3(KTriangle, KRectangle, KLineSegment)>
{
  public:
                            ...
                            std::vector<KThreeVector>& centerPoints)
{
    ...
    for (unsigned int i = 0; i < containerSize; i++) {
        if (shapeTypes[i].X() == 1.) {  // triangle
            ...
            if (distanceRatioTri < distanceRatioTriMin)
                distanceRatioTriMin = distanceRatioTri;
            if (distanceRatioTri > distanceRatioTriMax)
        ...
        if (shapeTypes[i].Y() == 1.) {  // rectangle
            ...
            if (distanceRatioRect < distanceRatioRectMin)
                distanceRatioRectMin = distanceRatioRect;
            if (distanceRatioRect > distanceRatioRectMax)
        ...
        if (shapeTypes[i].Z() == 1.) {  // line segment
            ...
            if (distanceRatioLine < distanceRatioLineMin)
                distanceRatioLineMin = distanceRatioLine;

            if (distanceRatioLine < distanceRatioLineMin)
                distanceRatioLineMin = distanceRatioLine;
            if (distanceRatioLine > distanceRatioLineMax)
    ...
    KEMField::cout << "Min. distance ratio for triangles: " << distanceRatioTriMin << ", max: " << distanceRatioTriMax
                   << KEMField::endl;
    KEMField::cout << "Min. distance ratio for rectangles: " << distanceRatioRectMin
                                         ...
                                         std::vector<KThreeVector>& centroids)
{
    ...
    outStream << "Distribution for triangles, P=(" << selectedPoint.X() << " | " << selectedPoint.Y() << " | "
              << selectedPoint.Z() << ") ;distance ratio;no. of triangles";
    ...
    outStream << "Distribution for rectangles, P=(" << selectedPoint.X() << " | " << selectedPoint.Y() << " | "
              << selectedPoint.Z() << ") ;distance ratio;no. of rectangles";
    ...
    outStream << "Distribution for line segments, P=(" << selectedPoint.X() << " | " << selectedPoint.Y() << " | "
              << selectedPoint.Z() << ") ;distance ratio;no. of line segments";
