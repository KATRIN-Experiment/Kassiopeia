...
private:
    ...
    KEMThreeVector fShapeType; /* ( triangle, rectangle, line segment ) */
};
} /* KEMField namespace*/

void GetMinMaxDistanceRatio( double* retValues, KEMThreeVector fP, unsigned int containerSize, std::vector<KEMThreeVector> &shapeTypes,
        std::vector<double> &avgLengths, std::vector<KEMThreeVector> &centerPoints )
{
...
std::vector<TH1D*> ComputeDistanceRatios( KEMThreeVector selectedPoint,
        unsigned int contSize, std::vector<KEMThreeVector> &types, std::vector<double> &lengths, std::vector<KEMThreeVector> &centroids )
{
...
int main(int argc, char* argv[])
{
    ...
    KEMThreeVector fieldPoint( 0., 0., 0. );

    DistRatioVisitor fShapeVisitor;
    std::vector<double> lengths;
    std::vector<KEMThreeVector> centroids;
