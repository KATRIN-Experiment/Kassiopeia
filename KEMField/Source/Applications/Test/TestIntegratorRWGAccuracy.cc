...
// THIS IS NEVER USED
       ...
       // FOR WHAT IF IT IS NEVER USED?
        ...
        if( lineIndex==0 ) goOut = KThreeVector( m0[0], m0[1], m0[2] ); // Unit vectors ???????
        if( lineIndex==1 ) goOut = KThreeVector( m1[0], m1[1], m1[2] );
        if( lineIndex==2 ) goOut = KThreeVector( m2[0], m2[1], m2[2] );

        KThreeVector goAlong;

        if( lineIndex==0 ) goAlong = KThreeVector( triAlongSideP0P1Unit[0], triAlongSideP0P1Unit[1], triAlongSideP0P1Unit[2] );
        if( lineIndex==1 ) goAlong = KThreeVector( triAlongSideP1P2Unit[0], triAlongSideP1P2Unit[1], triAlongSideP1P2Unit[2] );
        if( lineIndex==2 ) goAlong = KThreeVector( triAlongSideP2P0Unit[0], triAlongSideP2P0Unit[1], triAlongSideP2P0Unit[2] );
