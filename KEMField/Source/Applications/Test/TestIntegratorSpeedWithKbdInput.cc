...
double totalTime=0.;
    ...
    StartTimer();
#ifdef KEMFIELD_USE_OPENCL
        fOpenCLIntegratingFieldSolver->ElectricFieldAndPotential( KThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
        //fOpenCLIntegratingFieldSolver->Potential( KThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
#else
        fIntegratingFieldSolver->ElectricFieldAndPotentialNoKahanSum( KThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
        totalTime+=Time();
#endif
    }
