...
class TriangleVisitor : public KSelectiveVisitor<KShapeVisitor, KTYPELIST_1(KTriangle)>
{
  public:
...
class TriangleVisitorForElectricFieldAndPotential : public KSelectiveVisitor<KShapeVisitor, KTYPELIST_1(KTriangle)>
{
  public:
    using KSelectiveVisitor<KShapeVisitor, KTYPELIST_1(KTriangle)>::Visit;
    ...
    void ComputeElectricFieldAndPotential(KTriangle& t)
    {
        ...
        fQuadElectricFieldAndPotential =
            std::make_pair(fQuadIntegrator.ElectricField(&t, fP), fQuadIntegrator.Potential(&t, fP));

        fAnaElectricFieldAndPotential =
            std::make_pair(fAnaIntegrator.ElectricField(&t, fP), fAnaIntegrator.Potential(&t, fP));
        fRwgElectricFieldAndPotential =
            std::make_pair(fRwgIntegrator.ElectricField(&t, fP), fRwgIntegrator.Potential(&t, fP));

        fCub4ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 4, triQ4, gTriCub4w),
                           fCubIntegrator.Potential_TriNP(data, fP, 4, triQ4, gTriCub4w));
        fCub7ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 7, triQ7, gTriCub7w),
                           fCubIntegrator.Potential_TriNP(data, fP, 7, triQ7, gTriCub7w));
        fCub12ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 12, triQ12, gTriCub12w),
                           fCubIntegrator.Potential_TriNP(data, fP, 12, triQ12, gTriCub12w));
        fCub16ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 16, triQ16, gTriCub16w),
                           fCubIntegrator.Potential_TriNP(data, fP, 16, triQ16, gTriCub16w));
        fCub19ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 19, triQ19, gTriCub19w),
                           fCubIntegrator.Potential_TriNP(data, fP, 19, triQ19, gTriCub19w));
        fCub33ElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField_TriNP(data, fP, 33, triQ33, gTriCub33w),
                           fCubIntegrator.Potential_TriNP(data, fP, 33, triQ33, gTriCub33w));

        fNumElectricFieldAndPotential =
            std::make_pair(fCubIntegrator.ElectricField(&t, fP), fCubIntegrator.Potential(&t, fP));
#else
        ...
        fCub12ElectricFieldAndPotential =
            fCubIntegrator.ElectricFieldAndPotential_TriNP(data, fP, 12, triQ12, gTriCub12w);
        fCub16ElectricFieldAndPotential =
            fCubIntegrator.ElectricFieldAndPotential_TriNP(data, fP, 16, triQ16, gTriCub16w);
        fCub19ElectricFieldAndPotential =
            fCubIntegrator.ElectricFieldAndPotential_TriNP(data, fP, 19, triQ19, gTriCub19w);
        fCub33ElectricFieldAndPotential =
            fCubIntegrator.ElectricFieldAndPotential_TriNP(data, fP, 33, triQ33, gTriCub33w);
...
int main()
{
    ...
    for (unsigned int k = 0; k <= kmax; k++) {
        ...
        for (it = container->begin<KElectrostaticBasis>(); it != container->end<KElectrostaticBasis>(); ++it) {
            ...
            fP = fTriangleVisitor.GetCentroid() +
                 fTriangleVisitor.GetAverageSideLength() * Dr * fPointDirections[directionIndex];
        ...
        if (PLOTCUB16)
            plotDrPotCub16->SetPoint(k, Dr, relCub16Pot);
        if (PLOTCUB19)
            plotDrPotCub19->SetPoint(k, Dr, relCub19Pot);
        if (PLOTCUB33)
        ...
        if (PLOTCUB16)
            plotDrFieldCub16->SetPoint(k, Dr, relCub16Field);
        if (PLOTCUB19)
            plotDrFieldCub19->SetPoint(k, Dr, relCub19Field);
        if (PLOTCUB33)
    ...
    KEMField::cout << "Recommended distance ratio values for target accuracy " << targetAccuracy << " (+" << 100 * drAdd
                   << "%):" << KEMField::endl;
