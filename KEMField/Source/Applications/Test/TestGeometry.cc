...
int main()
{
    ...
    KSurface<KElectrostaticBasis, KDirichletBoundary, KRectangle>* rClone = r->Clone();
    ...
    std::cout << "There are " << fundamentalTypeCounter.NumberOfType<double>() << " doubles in " << w->Name()
              << std::endl;
    ...
    KSurface<KElectrostaticBasis, KDirichletBoundary, KTriangle>* another_t = t->Clone();
    ...
    KEMField::cout << "There are " << surfaceContainer.size<KDirichletBoundary, KTriangle>()
    ...
    t = (KSurface<KElectrostaticBasis, KDirichletBoundary, KTriangle>*) sP;

    KEMField::cout << "There are " << surfaceContainer.size<KDirichletBoundary>()
                   << " dirichlet elements in the container" << KEMField::endl;
    KEMField::cout << "There are " << surfaceContainer.size<KTriangle>() << " triangles in the container"
                   << KEMField::endl;
...

    {
        ...
        KEMFileInterface::GetInstance()->Write("testFile.kbd", surfaceContainer, "a_surface_container");

        KSurfaceContainer anotherSurfaceContainer;

        KEMFileInterface::GetInstance()->Read("testFile.kbd", anotherSurfaceContainer, "a_surface_container");
...

    {
        ...
        for (it = surfaceContainer.begin<KLineSegment>(); it != surfaceContainer.end<KLineSegment>(); it++)
            KEMField::cout << *(*it) << KEMField::endl;

        std::cout << "\n........\n\n" << std::endl;

        KEMField::cout << "Dirichlet elements:" << KEMField::endl;
        for (it = surfaceContainer.begin<KDirichletBoundary>(); it != surfaceContainer.end<KDirichletBoundary>(); it++)
            KEMField::cout << *(*it) << KEMField::endl;

        std::cout << "\n........\n\n" << std::endl;

        KEMField::cout << "Electrostatic elements:" << KEMField::endl;
        for (it = surfaceContainer.begin<KElectrostaticBasis>(); it != surfaceContainer.end<KElectrostaticBasis>();
             it++)
            KEMField::cout << *(*it) << KEMField::endl;
    ...
    KSurface<KElectrostaticBasis, KDirichletBoundary, KTriangle>* t2 = t->Clone();
