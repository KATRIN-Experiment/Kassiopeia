...
namespace KEMField
{
template<class DataStreamer> class KSerializer;
...
template<class DataStreamer> class KSerializerBase
{
  public:
    ...
    DataStreamer& GetDataStreamer()
    {
        return fDataStreamer;
    }
...
template<typename Type, class DataStreamer> class KSerializerType : virtual public KSerializerBase<DataStreamer>
{
  public:
...
template<class DataStreamer>
void KSerializer<DataStreamer>::open(const std::string& fileName, const std::string& action)
{
    std::stringstream s;
...
template<class DataStreamer> void KSerializer<DataStreamer>::close()
{
    GetMetadataStreamer().close();
    GetDataStreamer().close();
}

template<class DataStreamer> template<class Streamed> void KSerializer<DataStreamer>::PreStreamInAction(Streamed& s)
{
    GetMetadataStreamer().PreStreamInAction(s);
    GetDataStreamer().PreStreamInAction(s);
}

template<class DataStreamer> template<class Streamed> void KSerializer<DataStreamer>::PostStreamInAction(Streamed& s)
{
