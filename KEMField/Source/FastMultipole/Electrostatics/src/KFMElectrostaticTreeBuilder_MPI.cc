...
void
KFMElectrostaticTreeBuilder_MPI::SetElectrostaticElementContainer(KFMElectrostaticElementContainerBase<KFMELECTROSTATICS_DIM,KFMELECTROSTATICS_BASIS>* container)
{
...
void
KFMElectrostaticTreeBuilder_MPI::SetTree(KFMElectrostaticTree* tree)
{
    fTree = tree;
    ...
    if(fVerbosity > 1)
    {
        MPI_SINGLE_PROCESS
        {
            ...
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: top level divisions set to "<<params.top_level_divisions<<kfmendl;
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: divisions set to "<<params.divisions<<kfmendl;
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: degree set to "<<params.degree<<kfmendl;
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: zero mask size set to "<<params.zeromask<<kfmendl;
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: max tree depth set to "<<params.maximum_tree_depth<<kfmendl;

            if( !(params.use_region_estimation) )
            {
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: using user defined world cube volume"<<kfmendl;
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: world cube center set to ("<<params.world_center_x<<", "<<params.world_center_y<<", "<<params.world_center_z<<") "<<kfmendl;
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: world cube side length set to ("<<params.world_length<<kfmendl;
            }
            else
            {
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: using region size estimation"<<kfmendl;
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::SetParameters: region expansion factor set to "<<params.region_expansion_factor<<kfmendl;
            }
        }
    }
...
KFMElectrostaticTreeBuilder_MPI::GetTree()
{
    return fTree;
};
...
void
KFMElectrostaticTreeBuilder_MPI::ConstructRootNode()
{
    ...
    if(fUseRegionEstimation)
    {
        ...
        if(fVerbosity > 2)
        {
            MPI_SINGLE_PROCESS
            {
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::Initialize: Estimated world cube center is ("<<fWorldCenter[0]<<", "<<fWorldCenter[1]<<", "<<fWorldCenter[2]<<")"<<kfmendl;
                kfmout<<"KFMElectrostaticTreeBuilder_MPI::Initialize: Estimated world cube size length is "<<fWorldLength<<kfmendl;
            }
        }
    }
    ...
    if(fVerbosity > 2)
    {
        MPI_SINGLE_PROCESS
        {
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::ConstructRootNode: Constructing root node with center at ("<<center[0]<<", "<<center[1]<<", "<<center[2]<<")."<<kfmendl;
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::ConstructRootNode: Root node has side length of "<<fWorldLength<<" and contains "<<n_elements<<" boundary elements."<<kfmendl;
        }
    }
...
void
KFMElectrostaticTreeBuilder_MPI::PerformSpatialSubdivision()
{
    fTree->RestrictActionBehavior(false);
    ...
    if(fVerbosity > 2)
    {
        MPI_SINGLE_PROCESS
        {
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::PerformSpatialSubdivision: Subdividing region using the "<<fSubdivisionCondition->Name()<<" strategy "<<kfmendl;
        }
    }
    ...
    if(fVerbosity > 2)
    {
        MPI_SINGLE_PROCESS
        {
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::PerformSpatialSubdivision: Done performing spatial subdivision. "<<kfmendl;
        }
    }
}
...
void
KFMElectrostaticTreeBuilder_MPI::FlagNonZeroMultipoleNodes()
{
    fTree->RestrictActionBehavior(false);
...
void
KFMElectrostaticTreeBuilder_MPI::PerformAdjacencySubdivision()
{
    fTree->RestrictActionBehavior(false);
    ...
    if(fVerbosity > 3)
    {
        MPI_SINGLE_PROCESS
        {
            kfmout<<"KFMElectrostaticTreeBuilder_MPI::PerformAdjacencySubdivision: Done performing subdivision of nodes satisfying adjacency condition."<<kfmendl;
        }
    }
...
void
KFMElectrostaticTreeBuilder_MPI::FlagPrimaryNodes()
{
    fTree->RestrictActionBehavior(false);
...
void
KFMElectrostaticTreeBuilder_MPI::DetermineSourceNodes()
{
    ...
    if(fSubdivisionCondition->Name() == std::string("balanced") )
    {
        ...
        sbdc = dynamic_cast< KFMSubdivisionConditionBalanced< KFMELECTROSTATICS_DIM, KFMElectrostaticNodeObjects >* >(fSubdivisionCondition);
    ...
    for(unsigned int i=0; i<n_top_level_source_nodes; i++)
    {
        ...
        KFMObjectRetriever<KFMElectrostaticNodeObjects, KFMCube<KFMELECTROSTATICS_DIM> >::GetNodeObject(node)->GetCenter();
    ...
    if(KMPIInterface::GetInstance()->SplitMode())
    {
        if( KMPIInterface::GetInstance()->IsEvenGroupMember() )
        {
            ...
            if(ret_val != MPI_SUCCESS)
            {
                ...
                kfmout<<"KFMElectrostaticTreeBuilder:: Error using MPI_Send from process: "<<proc<<" to "<<partner<<kfmendl;
                KMPIInterface::GetInstance()->Finalize();
                std::exit(1);
            }

            ret_val = MPI_Send(&(process_block_ids[0]), n_block_ids, MPI_UNSIGNED, partner, NODELIST_ASSIGN_TAG, MPI_COMM_WORLD);
        ...
        else
        {
            ...
            MPI_Status stat;
            int ret_val = MPI_Recv(&n_block_ids, 1, MPI_UNSIGNED, KMPIInterface::GetInstance()->GetPartnerProcessID(), NODELIST_SIZE_TAG, MPI_COMM_WORLD, &stat);

            if(ret_val != MPI_SUCCESS)
            {
                int proc = KMPIInterface::GetInstance()->GetProcess();
                kfmout<<"KFMElectrostaticTreeBuilder:: Error using MPI_Recv to process: "<<proc<<" from "<<partner<<kfmendl;
                KMPIInterface::GetInstance()->Finalize();
                std::exit(1);
            }

            process_block_ids.resize(n_block_ids);
            ret_val = MPI_Recv(&(process_block_ids[0]), n_block_ids, MPI_UNSIGNED, KMPIInterface::GetInstance()->GetPartnerProcessID(), NODELIST_ASSIGN_TAG, MPI_COMM_WORLD, &stat);
    ...
    if(fNSourceNodes == 0)
    {
        ...
        kfmout<<"KFMElectrostaticTreeBuilder_MPI::DetermineSourceNodes(): Error, too many MPI processes allocated for this job. ";
        kfmout<<"The total number of top level source nodes is: "<<n_top_level_source_nodes<<" but the number of processes is: "<<n_processes<<". ";
    ...
    {
        std::stringstream ss;
        ss << "Process #"<< KMPIInterface::GetInstance()->GetProcess() <<" has been assigned source nodes w/ child indexes: {";
        for(unsigned int i=0; i<fSourceNodeCollection.size()-1; i++){ ss << fSourceNodeCollection[i]->GetIndex() <<", "; };
    ...
    for(unsigned int i=0; i<fNSourceNodes; i++)
    {
        ...
        cube = KFMObjectRetriever<KFMElectrostaticNodeObjects, KFMCube<KFMELECTROSTATICS_DIM> >::GetNodeObject(fSourceNodeCollection[i]);
    ...
    for(unsigned int i=0; i<n_children; i++)
    {
        ...
        {
            if(fSourceNodeCollection[j] == node){non_source = false;};
        }
...
void
KFMElectrostaticTreeBuilder_MPI::GetSourceNodeIndexes(std::vector<unsigned int>* source_node_indexes) const
{
...
void
KFMElectrostaticTreeBuilder_MPI::GetTargetNodeIndexes(std::vector<unsigned int>* target_node_indexes) const
{
...
void
KFMElectrostaticTreeBuilder_MPI::DetermineTargetNodes()
{
    ...
    {
        std::stringstream ss;
        ss << "Process #"<<KMPIInterface::GetInstance()->GetProcess()<<" has been assigned target nodes w/ child indexes: { ";
        for(unsigned int i=0; i<fTargetNodeCollection.size()-1; i++){ ss << fTargetNodeCollection[i]->GetIndex() <<", "; };
    ...
    {
        KFMCube<KFMELECTROSTATICS_DIM>* cube = NULL;
        cube = KFMObjectRetriever<KFMElectrostaticNodeObjects, KFMCube<KFMELECTROSTATICS_DIM> >::GetNodeObject(fTargetNodeCollection[i]);
    ...
    for(unsigned int i=0; i<n_children; i++)
    {
        ...
        {
            if(fTargetNodeCollection[j] == node){non_target = false;};
        }
...
void
KFMElectrostaticTreeBuilder_MPI::RemoveExtraneousData()
{
    ...
    KFMNodeChildToParentFlagValueInitializer<KFMElectrostaticNodeObjects, KFMELECTROSTATICS_FLAGS> root_source_flag_init;
    ...
    KFMNodeObjectNullifier<KFMElectrostaticNodeObjects, KFMElectrostaticElementContainerBase<KFMELECTROSTATICS_DIM, KFMELECTROSTATICS_BASIS> > elementContainerNullifier;
    KFMNodeObjectNullifier<KFMElectrostaticNodeObjects, KFMCubicSpaceTreeProperties< KFMELECTROSTATICS_DIM > > treePropertyNullifier;
    ...
    for(unsigned int i=0; i<fNonTargetNodeCollection.size(); i++)
    {
        ...
        KFMObjectRetriever<KFMElectrostaticNodeObjects, KFMElectrostaticElementContainerBase<KFMELECTROSTATICS_DIM, KFMELECTROSTATICS_BASIS> >::SetNodeObject(fContainer, fNonTargetNodeCollection[i]);
        ...
        tree_prop = KFMObjectRetriever<KFMElectrostaticNodeObjects,  KFMCubicSpaceTreeProperties< KFMELECTROSTATICS_DIM > >::GetNodeObject(fTree->GetRootNode());
        KFMObjectRetriever<KFMElectrostaticNodeObjects,  KFMCubicSpaceTreeProperties< KFMELECTROSTATICS_DIM > >::SetNodeObject(tree_prop, fNonTargetNodeCollection[i]);
...
void
KFMElectrostaticTreeBuilder_MPI::CollectDirectCallIdentitiesForPrimaryNodes()
{
    fTree->RestrictActionBehavior(false);
