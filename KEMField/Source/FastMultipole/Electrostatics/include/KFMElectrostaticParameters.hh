...
struct KFMElectrostaticParameters
{
    KFMElectrostaticParameters():
        ...
        region_expansion_factor(1.1), // if auto region esimation is on, the multiplication factor governing the size of the region
  ...
  Stream& operator>>(Stream& s, KFMElectrostaticParameters& p)
  {
    ...
    s >> i; p.strategy = i;
    s >> i; p.top_level_divisions = i;
    s >> i; p.divisions = i;
    s >> i; p.degree = i;
    s >> i; p.zeromask = i;
    s >> i; p.maximum_tree_depth = i;
    s >> d; p.insertion_ratio = d;
    ...
    s >> i; p.allowed_number = i;
    s >> d; p.allowed_fraction = d;
    s >> d; p.bias_degree = d;
