...
template<typename ObjectTypeList, unsigned int SpatialNDIM> class KFMCollocationPointIdentitySetCreator
{
  public:
    ...
    void SortCollocationPoints(const std::vector<const KFMPoint<SpatialNDIM>*>* c_points)
    {
        ...
        for (unsigned int id = 0; id < size; id++) {
            ...
            if (fNavigator.Found()) {
                ...
                //warning, abort!
                kfmout
                    << "KFMCollocationPointIdentitySetCreator::SortCollocationPoints: Abort! Point not found in tree region. "
                    << kfmendl;
                kfmexit(1);
            }
        }
    ...
    void SortCollocationPoints(const std::vector<const KFMPoint<SpatialNDIM>*>* c_points,
                               const std::vector<unsigned int>* c_point_ids)
    {
        //we assume that the placement of the collocation point in the list is its identity index
        unsigned int size = c_points->size();
