...
template<unsigned int NDIM> struct KFMWorkBlock
{
    unsigned int index;
...
template<unsigned int NDIM> bool KFMWorkBlockCompareMortonZOrder(KFMWorkBlock<NDIM> a, KFMWorkBlock<NDIM> b)
{
    return (a.z_order_index < b.z_order_index);
};

template<unsigned int NDIM> class KFMCubicSpaceTreeStaticLoadBalancer
{
  public:
  ...
  protected:
    ...
    double ComputeProcessEnergyFunction(
        const std::list<KFMWorkBlock<NDIM>>* proc_blocks);  //computes energy of single process state
...
template<unsigned int NDIM> KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::KFMCubicSpaceTreeStaticLoadBalancer()
{
...
template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::SetBlockLength(double len)
{
    fBlockLength = len;
};

template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::SetDivisions(unsigned int div)
{
    fDivisions = div;
    fNMaxBlocks = 1;
    for (unsigned int i = 0; i < NDIM; i++) {
...
template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::SetNeighborOrder(unsigned int order)
{
    fNeighborOrder = order;
}

template<unsigned int NDIM>
void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::SetBlocks(const std::vector<KFMWorkBlock<NDIM>>* work_blocks)
{
    fAllBlocks = *work_blocks;
    fNBlocks = fAllBlocks.size();
}

template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::Initialize()
{
    ...
    for (unsigned int i = 0; i < fNBlocks; i++) {
        if (fAllBlocks[i].score > max_score) {
            max_score = fAllBlocks[i].score;
        };
    }
    ...
    if (fVerbosity >= 3) {
        if (fProcessID == 0) {
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::Initialize: Block score mean: " << fMeanBlockScore
                   << kfmendl;
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::Initialize: Block score variance: " << fBlockScoreVariance
                   << kfmendl;
        }
    }
}
...
template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::EstimateBalancedWork()
{
    ...
    if (fNProcesses < fNBlocks) {
        ...
        for (unsigned int i = 0; i < fNBlocks; i++) {
            ...
            if (process_n_workitems[i % fNProcesses] > fMaxWorkitems) {
                fMaxWorkitems = process_n_workitems[i % fNProcesses];
            };
        }
        ...
        if (fVerbosity >= 3) {
            if (fProcessID == 0) {
                ...
                for (unsigned int i = 0; i < fNProcesses; i++) {
                    if (max_proc_score < proc_score[i]) {
                        max_proc_score = proc_score[i];
                    };
                    if (min_proc_score > proc_score[i]) {
                        min_proc_score = proc_score[i];
                    };
                ...
                kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::EstimateBalancedWork: Initial max process score: "
                       << max_proc_score << kfmendl;
                kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::EstimateBalancedWork: Initial min process score: "
                       << min_proc_score << kfmendl;
                kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::EstimateBalancedWork: Initial process score mean: "
                       << ave_proc_score << kfmendl;
                kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::EstimateBalancedWork: Initial process score variance: "
                       << var_proc_score << kfmendl;
            }
        }
...
//seed rand with process id
        ...
        kfmout
            << "KFMCubicSpaceTreeStaticLoadBalancer::EstimateBalancedWork(): Error, too many MPI processes allocated for this job. ";
...
//functions needed for simulated annealing
template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::PermuteState()
{
...
template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::UpdateState()
{
    for (unsigned int i = 0; i < fNProcesses; i++) {
...
template<unsigned int NDIM>
double KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::UniformRandom(double lower_limit, double upper_limit)
{
    double r = 0;
#ifndef KEMFIELD_USE_GSL
...
template<unsigned int NDIM> double KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::ComputePermutedStateEnergyFunction()
{
    double max_proc_score = 0.0;
    for (unsigned int i = 0; i < fNProcesses; i++) {
        double proc_score = ComputeProcessEnergyFunction(&(fPermutedProcessBlockSets[i]));
        if (proc_score > max_proc_score) {
            max_proc_score = proc_score;
        };
    }
...
template<unsigned int NDIM>
double KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::ComputeProcessEnergyFunction(
    const std::list<KFMWorkBlock<NDIM>>* proc_blocks)
{
...
template<unsigned int NDIM> double KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::ComputeTemperatureFunction(int iteration)
{
    return fInitialTemperature * std::pow(fGamma, iteration);
}


template<unsigned int NDIM> void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::DetermineBestAvailableState()
{
    ...
    if (fMaxWorkitems == 1) {
        ...
        best_process_id = 0;
        for (unsigned int i = 0; i < fNProcesses; i++) {
            if (energies_out[i] < min_energy) {
                min_energy = energies_out[i];
                best_process_id = i;
            };
        }
    }
    ...
    if (fVerbosity >= 3) {
        if (fProcessID == best_process_id) {
            ...
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::DetermineBestAvailableState: Final max process score: "
                   << max_proc_score << kfmendl;
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::DetermineBestAvailableState: Final min process score: "
                   << min_proc_score << kfmendl;
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::DetermineBestAvailableState: Final process score mean: "
                   << ave_proc_score << kfmendl;
            kfmout << "KFMCubicSpaceTreeStaticLoadBalancer::DetermineBestAvailableState: Final process score variance: "
                   << var_proc_score << kfmendl;
        }
    }
...
template<unsigned int NDIM>
void KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::GetProcessBlockIdentities(
    std::vector<unsigned int>* process_block_ids) const
{
...
template<unsigned int NDIM>
double KFMCubicSpaceTreeStaticLoadBalancer<NDIM>::BlockCenterDistance(const double* p1, const double* p2)
{
    double val = 0.0;
    for (unsigned int i = 0; i < NDIM; i++) {
