...
std::complex<double> KFMResponseKernel_3DLaplaceM2M::GetResponseFunction(int source_index, int target_index) const
{
    ...
    pre *= (KFMMath::A_Coefficient(m_prime, n_prime) * KFMMath::A_Coefficient(k - m_prime, j - n_prime)) /
           (KFMMath::A_Coefficient(k, j));

    return pre * (KFMMath::RegularSolidHarmonic_Cart(n_prime, -1 * m_prime, fDel));
}


std::complex<double> KFMResponseKernel_3DLaplaceM2M::GetSourceScaleFactor(int source_index,
...
std::complex<double> KFMResponseKernel_3DLaplaceM2M::GetTargetScaleFactor(int target_index,
                                                                          std::complex<double>& scale) const
{
    int j;
    j = KFMScalarMultipoleExpansion::ComplexDegreeReverseLookUp(target_index);
...
std::complex<double> KFMResponseKernel_3DLaplaceM2M::GetNormalizationFactor(int source_index, int target_index) const
{
    //Note that the index convention here has been changed,
    //as compared with that of equation (5.22) of the paper:
    //"A Short course on fast multipole methods" by R. Beatson, and L. Greengard
...
std::complex<double> KFMResponseKernel_3DLaplaceM2M::GetIndependentResponseFunction(int response_index) const
{
    int j, k;
