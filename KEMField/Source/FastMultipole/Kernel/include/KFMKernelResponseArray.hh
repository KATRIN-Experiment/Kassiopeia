...
class KFMKernelResponseArray : public KFMArrayFillingOperator<std::complex<double>, SpatialNDIM + 2>
{
  ...
  public:
    ...
    ~KFMKernelResponseArray() override = default;
    ...
    void Initialize() override
    {
        ...
        if (this->fOutput != nullptr) {
            if (this->IsBoundedDomainSubsetOfArray(this->fOutput, fLowerLimits, fUpperLimits)) {
                ...
                kfmout
                    << "KFMKernelResponseArray::Initialize: error, not properly initialized, array bounds/bases incorrect."
                    << kfmendl;
            }
        }
        else {
            kfmout << "KFMKernelResponseArray::Initialize: error, initialization failed, output array not set."
                   << kfmendl;
        }
    }
