...
void KFMComplexSphericalHarmonicExpansionRotator::SetDegree(int l_max)
{
    ...
    if (fNormalizationCoefficients != nullptr) {
        delete[] fNormalizationCoefficients;
        fNormalizationCoefficients = nullptr;
    };
    fNormalizationCoefficients = new double[fSize];

    if (fInverseNormalizationCoefficients != nullptr) {
        delete[] fInverseNormalizationCoefficients;
        fInverseNormalizationCoefficients = nullptr;
    };
...
void KFMComplexSphericalHarmonicExpansionRotator::SetJMatrices(const std::vector<kfm_matrix*>* j_matrix)
{
    fRealRotator->SetJMatrices(j_matrix);
}

bool KFMComplexSphericalHarmonicExpansionRotator::IsValid()
{
    return fRealRotator->IsValid();
}

void KFMComplexSphericalHarmonicExpansionRotator::SetSingleZRotationAngle(double alpha)
{
...
void KFMComplexSphericalHarmonicExpansionRotator::SetEulerAngles(double alpha, double beta, double gamma)
{
    fRealRotator->SetEulerAngles(alpha, beta, gamma);
}

void KFMComplexSphericalHarmonicExpansionRotator::SetMoments(const std::vector<std::complex<double>>* mom)
{
...
void KFMComplexSphericalHarmonicExpansionRotator::Rotate()
{
    fRealRotator->Rotate();
}

void KFMComplexSphericalHarmonicExpansionRotator::GetRotatedMoments(std::vector<std::complex<double>>* mom)
{
    if (mom->size() != fSize) {
        mom->resize(fSize);
    }
