...
void KFMPinchonJMatrixCalculator::AllocateMatrices(std::vector<kfm_matrix*>* matrices)
{
...
bool KFMPinchonJMatrixCalculator::ComputeMatrices(std::vector<kfm_matrix*>* matrices)
{
    if (matrices == nullptr) {
        return false;
    }
...
void KFMPinchonJMatrixCalculator::ComputeJMatrixFromPrevious(unsigned int target_degree, kfm_matrix* prev,
                                                             kfm_matrix* target)
{
...
bool KFMPinchonJMatrixCalculator::CheckMatrixSizes(std::vector<kfm_matrix*>* matrices)
{
    for (unsigned int l = 0; l < matrices->size(); l++) {
        ...
        if (nrows != matrices->at(l)->size1) {
            return false;
        };
        if (ncolumns != matrices->at(l)->size2) {
            return false;
        };
    }
    return true;
