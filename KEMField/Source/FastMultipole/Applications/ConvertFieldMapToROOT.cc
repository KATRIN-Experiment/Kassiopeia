...
int main(int argc, char** argv)
{
    ...
    if(use_x && use_y && use_z){mode = 0;}; //histogram all data
    if(!use_x && !use_y && !use_z){mode = 0;}; //histogram all data
    if(use_x && !use_y && !use_z){mode = 1;}; //1d plot as function of x
    if(!use_x && use_y && !use_z){mode = 2;}; //1d plot as function of y
    if(!use_x && !use_y && use_z){mode = 3;}; //1d plot as function of z
    if(use_x && use_y && !use_z){mode = 4;}; //2d plot as function of (x,y)
    if(!use_x && use_y && use_z){mode = 5;}; //2d plot as function of (y,z)
    if(use_x && !use_y && use_z){mode = 6;}; //2d plot as function of (x,z)
        ...
        if( (name != (std::string("x_coordinate") ) ) && (name != (std::string("y_coordinate") ) ) && (name != (std::string("z_coordinate") ) ) )
        {
                ...
                (name != (std::string("direct_time_per_field_call") ) )   )
            {
                ...
                if(mode == 4)
                {
                    ...
                    for(unsigned int j=0; j<size; j++)
                    {
                        g->SetPoint(j, x_coord->GetValue(j), y_coord->GetValue(j), data->GetDataSetWithIndex(i)->GetValue(j) );
                    }
                    g->SetMarkerStyle(24);
                    g->Draw("PCOLZ");
                    c->Update();
                ...
                if(mode == 5)
                {
                    ...
                    for(unsigned int j=0; j<size; j++)
                    {
                        g->SetPoint(j, y_coord->GetValue(j), z_coord->GetValue(j), data->GetDataSetWithIndex(i)->GetValue(j) );
                    }
                    g->SetMarkerStyle(24);
                    g->Draw("PCOLZ");
                    c->Update();
                ...
                if(mode == 6)
                {
                    ...
                    for(unsigned int j=0; j<size; j++)
                    {
                        g->SetPoint(j, x_coord->GetValue(j), z_coord->GetValue(j), data->GetDataSetWithIndex(i)->GetValue(j) );
                    }
                    g->SetMarkerStyle(24);
                    g->Draw("PCOLZ");
                    c->Update();
