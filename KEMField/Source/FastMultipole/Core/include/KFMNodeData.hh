...
class KFMNodeData : public KSAFixedSizeInputOutputObject
{
  public:
    ...
    void DefineOutputNode(KSAOutputNode* node) const override
    {
        if (node != nullptr) {
            node->AddChild(
                new KSAAssociatedValuePODOutputNode<KFMNodeData, unsigned int, &KFMNodeData::GetID>(std::string("id"),
                                                                                                    this));
            node->AddChild(
                new KSAAssociatedPassedPointerPODOutputNode<KFMNodeData,
                                                            std::vector<unsigned int>,
                                                            &KFMNodeData::GetChildIDs>(std::string("child_ids"), this));
        }
    }
    ...
    void DefineInputNode(KSAInputNode* node) override
    {
        ...
        if (node != nullptr) {
            node->AddChild(new KSAAssociatedReferencePODInputNode<KFMNodeData, unsigned int, &KFMNodeData::SetID>(
                std::string("id"),
                this));
            node->AddChild(
                new KSAAssociatedPointerPODInputNode<KFMNodeData, std::vector<unsigned int>, &KFMNodeData::SetChildIDs>(
...
template<typename Stream> Stream& operator>>(Stream& s, KFMNodeData& aData)
{
...
template<typename Stream> Stream& operator<<(Stream& s, const KFMNodeData& aData)
{
