...
//	r = gsl_rng_alloc (T);
    ...
    for(unsigned int n=0; n < NGroups; n++)
    {
        ...
        {
            if( (test_points[i] - centroid).Magnitude() > radius ){radius = (test_points[i] - centroid).Magnitude(); };
        }
        ...
        if(min_ball.GetRadius() > radius)
        {
            ...
            for(unsigned int i=0; i<NPointsInGroup; i++)
            {
                std::cout<<"p"<<i<<" = "<<test_points[i][0]<<", "<<test_points[i][1]<<", "<<test_points[i][2]<<std::endl;
            }
