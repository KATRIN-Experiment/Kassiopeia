...
class ConfigureTestSingleTriangleMultipole : public KSAInputOutputObject
{
  public:
    ...
    {
        return "ConfigureTestSingleTriangleMultipole";
    };

    int GetUseAnalyticCalculator() const
    {
        return fUseAnalyticCalculator;
    };
    void SetUseAnalyticCalculator(const int& s)
    {
        fUseAnalyticCalculator = s;
    };
    ...
    void SetDegree(const std::vector<int>* deg)
    {
        fDegree = *deg;
    };
    ...
    void SetNTriangles(const int& s)
    {
        fNTriangles = std::fabs(s);
    };

    int GetNSamplePointsPerTriangle() const
    {
        return fNSamplePointsPerTriangle;
    };
    void SetNSamplePointsPerTriangle(const int& n)
    {
        fNSamplePointsPerTriangle = std::fabs(n);
    };

    double GetTriangleBoundingRadius() const
    {
        return fTriangleBoundingRadius;
    };
    void SetTriangleBoundingRadius(const double& d)
    {
        fTriangleBoundingRadius = std::fabs(d);
    };

    double GetMaxSampleRadius() const
    {
        return fMaxSampleRadius;
    };
    void SetMaxSampleRadius(const double& r)
    {
        fMaxSampleRadius = std::fabs(r);
    };

    int GetNQuadratureTerms() const
    {
        return fNQuadratureTerms;
    };
    void SetNQuadratureTerms(const int& r)
    {
    ...
    void DefineOutputNode(KSAOutputNode* node) const override
    {
        ...
        node->AddChild(new KSAAssociatedPointerPODOutputNode<ConfigureTestSingleTriangleMultipole,
                                                             std::vector<int>,
                                                             &ConfigureTestSingleTriangleMultipole::GetDegree>(
            std::string("Degree"),
            this));
    }
    ...
    void DefineInputNode(KSAInputNode* node) override
    {
        ...
        node->AddChild(new KSAAssociatedPointerPODInputNode<ConfigureTestSingleTriangleMultipole,
                                                            std::vector<int>,
                                                            &ConfigureTestSingleTriangleMultipole::SetDegree>(
            std::string("Degree"),
            this));
    }
...
int main(int argc, char* argv[])
{
    ...
    KSAObjectInputNode<ConfigureTestSingleTriangleMultipole>* config_input =
        new KSAObjectInputNode<ConfigureTestSingleTriangleMultipole>(
            std::string("ConfigureTestSingleTriangleMultipole"));
    ...
    for (int i = 0; i < NTriangles; i++) {
        ...
        for (int j = 0; j < NSamplePointsPerTriangle; j++) {
            ...
            for (unsigned int k = 0; k < Degree.size(); k++) {
                ...
                p_direct =
                    (integrator.Potential(Triangles[count], point)) * (integrator.BasisValue(Triangles[count], 0));
                p_multipole = (PotentialMultipole(Degree[k], point, origin, &Moments)) *
                              (integrator.BasisValue(Triangles[count], 0)) * (Triangles[count]->Area());
