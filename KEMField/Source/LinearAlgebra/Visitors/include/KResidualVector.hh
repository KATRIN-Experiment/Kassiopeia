...
namespace KEMField
{
template<typename ValueType> class KResidualVector : public KSimpleVector<ValueType>
{
  public:
    ...
    {
        return std::string("ResidualVector");
    }

    void SetIteration(unsigned long long iteration)
    {
        fIteration = iteration;
    }
    unsigned long long GetIteration() const
    {
        return fIteration;
    }

    template<typename Stream> friend Stream& operator>>(Stream& s, KResidualVector& v)
    {
    ...
    template<typename Stream> friend Stream& operator<<(Stream& s, const KResidualVector& v)
    {
