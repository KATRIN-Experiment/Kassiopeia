...
template<typename ValueType> KRobinHood_MPI<ValueType>::~KRobinHood_MPI()
{
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::Initialize()
{
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::FindResidual()
{
    for (unsigned int i = 0; i < fResidual.Dimension(); i++)
        fResidual[i] = fB(ToGlobal(i)) - fB_iterative(i);
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::FindResidualNorm(double& residualNorm)
{
    residualNorm = 0.;
    for (unsigned int i = 0; i < fResidual.Dimension(); i++)
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::IdentifyLargestResidualElement()
{
    fMaxResidualIndex = 0;
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::IdentifyLargestResidualElement(Type2Type<double>)
{
...
void KRobinHood_MPI<ValueType>::IdentifyLargestResidualElement(Type2Type<std::complex<double>>)
{
    ...
    fRes_complex.fCorrection_real =
        ((fB(fMaxResidualIndex) - fMaxB_iterative) / fA(fMaxResidualIndex, fMaxResidualIndex)).real();
    fRes_complex.fCorrection_imag =
        ((fB(fMaxResidualIndex) - fMaxB_iterative) / fA(fMaxResidualIndex, fMaxResidualIndex)).imag();
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::ComputeCorrection()
{
    ComputeCorrection(Type2Type<ValueType>());
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::ComputeCorrection(Type2Type<double>)
{
    fCorrection = fRes_real.fCorrection;
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::ComputeCorrection(Type2Type<std::complex<double>>)
{
    fCorrection = std::complex<double>(fRes_complex.fCorrection_real, fRes_complex.fCorrection_imag);
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::UpdateSolutionApproximation()
{
    fX[fMaxResidualIndex] += fCorrection;
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::UpdateVectorApproximation()
{
    for (unsigned int i = 0; i < fB_iterative.Dimension(); i++)
        fB_iterative[i] += fA(ToGlobal(i), fMaxResidualIndex) * fCorrection;
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::Finalize()
{
    MPI_Barrier(MPI_COMM_WORLD);
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::SetResidualVector(const Vector& v)
{
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::GetResidualVector(Vector& v)
{
    GetResidualVector(Type2Type<ValueType>(), v);
}

template<typename ValueType> void KRobinHood_MPI<ValueType>::GetResidualVector(Type2Type<double>, Vector& v)
{
...
template<typename ValueType>
void KRobinHood_MPI<ValueType>::MPIRealMax(Res_Real* in, Res_Real* inout, int* len, MPI_Datatype*)
{
    int i;
...
template<typename ValueType>
void KRobinHood_MPI<ValueType>::MPIRealMin(Res_Real* in, Res_Real* inout, int* len, MPI_Datatype*)
{
    int i;
...
template<typename ValueType>
void KRobinHood_MPI<ValueType>::MPIComplexMax(Res_Complex* in, Res_Complex* inout, int* len, MPI_Datatype*)
{
    int i;
...
template<typename ValueType>
void KRobinHood_MPI<ValueType>::MPIComplexMin(Res_Complex* in, Res_Complex* inout, int* len, MPI_Datatype*)
{
    int i;
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::InitializeMPIStructs(Type2Type<double>)
{
...
template<typename ValueType> void KRobinHood_MPI<ValueType>::InitializeMPIStructs(Type2Type<std::complex<double>>)
{
