...
void KElectrostaticCubatureRectangleIntegrator::GaussPoints_Rect33P(const double* data, double* Q) const
{
    ...
    const double X[9] = {0.00000000000000000000,
    ...
    const double Y[9] = {0.00000000000000000000,
    ...
    const double x[33] = {X[0],  X[1],  -X[1], -Y[1], Y[1],  X[2],  -X[2], -Y[2], Y[2],  X[3],  -X[3],
                          -Y[3], Y[3],  X[4],  -X[4], -Y[4], Y[4],  X[5],  -X[5], -Y[5], Y[5],  X[6],
                          -X[6], -Y[6], Y[6],  X[7],  -X[7], -Y[7], Y[7],  X[8],  -X[8], -Y[8], Y[8]};

    const double y[33] = {Y[0],  Y[1],  -Y[1], X[1],  -X[1], Y[2],  -Y[2], X[2],  -X[2], Y[3],  -Y[3],
                          X[3],  -X[3], Y[4],  -Y[4], X[4],  -X[4], Y[5],  -Y[5], X[5],  -X[5], Y[6],
                          -Y[6], X[6],  -X[6], Y[7],  -Y[7], X[7],  -X[7], Y[8],  -Y[8], X[8],  -X[8]};
...
double KElectrostaticCubatureRectangleIntegrator::Potential(const KRectangle* source, const KPosition& P) const
{
    ...
    const double cen[3] = {data[2] + 0.5 * (data[0] * data[5] + data[1] * data[8]),
...
double KElectrostaticCubatureRectangleIntegrator::Potential(const KSymmetryGroup<KRectangle>* source,
                                                            const KPosition& P) const
{
    double potential = 0.;
