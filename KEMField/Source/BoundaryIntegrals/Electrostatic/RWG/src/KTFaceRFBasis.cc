    ...
    for (unsigned int iSurface = 0; iSurface < 1; iSurface++) {
        ...
        if (isElectric(iTarget)) {
            ...
            theBoundaryCondition =
                DotProduct<std::vector<Complex_t>>(CrossProduct<std::vector<Complex_t>>(theNormal, theField),
                                                   theCurrent);
            ;
            ...
            if (isCharge(i)) {
                ...
                if (norm(scale) != 0.) {
                    for (int index = 0; index < 3; index++)
                        aValue[index] += (alpha) * (pow(-1., iSurface + 1) * s_j) * (Nij[index]) / scale;
                }
            }
            else {
                ...
                if (norm(scale) != 0.) {
                    for (int index = 0; index < 3; index++)
                        aValue[index] += (alpha) * (pow(-1., iSurface + 1) * s_j) * (Sij[index]) * (-im * k) / scale;
                }
            }
        }
    }
        ...
        for (int iSurface = 0; iSurface < 2; iSurface++) {
            ...
            if (isCurrent(i) || isCurrentTangent(i)) {
                ...
                for (int index = 0; index < 3; index++) {
                    ...
                    if (norm(scale) != 0.)
                        aValue[index] -= (alpha) * (pow(-1., iSurface + 1) * s_j) * (Kij[index]) / scale;
                }
            }
        }
    }
...
Complex_t KTFaceRFBasis::GetMatrixElement(unsigned int iTarget, unsigned int iSource) const
{
    ...
    for (int iSurface = 0; iSurface < 2; iSurface++) {
        ...
        if (isElectric(iTarget) && isCharge(iSource)) {
            ...
            if (KroneckerDelta<KTElectrode*>(eTarget, eSource) == 1.)
                Lij = 0.5 * Iij;
            else
                Lij += DotProduct<std::vector<Complex_t>>(theNormal, Nij);
        }
        ...
        if ((!isElectric(iTarget)) && (!isCharge(iSource))) {
            ...
            if (KroneckerDelta<KTElectrode*>(eTarget, eSource) == 1.)
                Lij = 0.5 * DotProduct<std::vector<Complex_t>>(Jt, Js);
            else
                Lij += DotProduct<std::vector<Complex_t>>(CrossProduct<std::vector<Complex_t>>(theNormal, Kij), Jt);
        }
        aValue += (s_i * s_j) * Lij;
    }
...
Complex_t KTFaceRFBasis::GetSolutionElement(unsigned int iSource) const
{
    ...
    if (isCharge(iSource))
        aValue = eSource->GetElectricCurrentWeight(0);
    if (isCurrent(iSource))
        aValue = eSource->GetElectricCurrentWeight(1);
    if (isCurrentTangent(iSource))
        aValue = eSource->GetElectricCurrentWeight(2);
...
void KTFaceRFBasis::SetSolutionElement(unsigned int iSource, const Complex_t aValue) const
{
    ...
    if (isCharge(iSource))
        eSource->SetElectricCurrentWeight(0, aValue);
    if (isCurrent(iSource))
        eSource->SetElectricCurrentWeight(1, aValue);
    if (isCurrentTangent(iSource))
        eSource->SetElectricCurrentWeight(2, aValue);
}
...
std::vector<Complex_t> KTFaceRFBasis::GetSurfaceVector(unsigned int iTarget, const double* P) const
{
    ...
    if (isCurrentTangent(iTarget) || isMagneticTangent(iTarget))
        index = 1;
...
std::vector<Complex_t> KTFaceRFBasis::GreenFunctionIntegral(const unsigned int iType, double* R, double k,
                                                            unsigned int iElectrode) const
{
    ...
    if (theElement == NULL) {
        ...
        if (isCurrentTangent(iElectrode))
            jindex = 1;
        ...
        if (iType == kCurl) {
            ...
            for (unsigned int j = 0; j < 3; j++)
                theResult[j] = theResult[0] * theCurrent[j];
        }
        else {
        ...
        for (int iElement = 0; iElement < (int) theSource->GetNSubElements(); iElement++) {
            ...
            for (int j = 0; j < 3; j++)
                theResult[j] += pow(-1., iElement + 1) * F[j];
