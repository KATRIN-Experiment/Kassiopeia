...
class KTFaceRFBasis : public KTBasis
{
  public:
    ...
    template<typename SysVector> typename SysVector::value_type DotProduct(const SysVector& A, const SysVector& B) const
    {
    ...
    template<typename SysObject> double KroneckerDelta(const SysObject& A, const SysObject& B) const
    {
        return (A == B) ? 1. : 0.;
    };
  ...
  protected:
    ...
    bool isElectric(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 0) ? true : false;
    }
    bool isMagnetic(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 1) ? true : false;
    }
    bool isMagneticTangent(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 2) ? true : false;
    }

    //!	Determine if these are induced charges/currents components
    bool isCharge(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 0) ? true : false;
    }
    bool isCurrent(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 1) ? true : false;
    }
    bool isCurrentTangent(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) == 2) ? true : false;
    }

    unsigned int GetElectrodeSubIndex(unsigned int index) const
    {
        return ((unsigned int) (index / fNumElectrodes) % fDegreeOfFreedom);
    }

    std::vector<Complex_t> GreenFunctionIntegral(const unsigned int iType, double* R, double k,
    ...
    inline unsigned int GetType(unsigned int iTarget) const
    {
        ...
        if (isCharge(iTarget))
            theType = kElectricCharge;
        else
            theType = kElectricCurrent;
        return theType;
    };
    ...
    inline Complex_t GetCoefficient(unsigned int iType, KTElectrode* eTarget, unsigned int iSurface) const
    {
        ...
        if (iType == kElectricCharge && norm(eps) > 0.)
            aValue = sqrt(eps);
        if (iType == kMagneticCurrent && norm(eps) > 0.)
            aValue = 1. / sqrt(eps);
        if (iType == kElectricCurrent && norm(mu) > 0.)
            aValue = 1. / sqrt(mu);
        if (iType == kMagneticCharge && norm(mu) > 0.)
            aValue = sqrt(mu);
