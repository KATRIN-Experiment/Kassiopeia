...
namespace KEMField
{
  ...
  KThreeVector KMagnetostaticLineSegmentIntegrator::MagneticField(const KLineSegment* source,const KPosition& P) const
  {
    ...
    double prefac = (KEMConstants::Mu0OverPi/(4.*s)*
    		       (sinTheta1-sinTheta0));
    
    return r0.Cross(i).Unit()*prefac;
  }

    KThreeVector KMagnetostaticLineSegmentIntegrator::VectorPotential(const KSymmetryGroup<KLineSegment>* source, const KPosition& P) const
    {
      KThreeVector A;
    ...
    KThreeVector KMagnetostaticLineSegmentIntegrator::MagneticField(const KSymmetryGroup<KLineSegment>* source, const KPosition& P) const
    {
