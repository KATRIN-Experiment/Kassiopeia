 ...
 */
void AddRect(KSurfaceContainer &fContainer, int &fGroup, int &fChDen, double fA, double fB,
        KEMThreeVector fP0, KEMThreeVector fN1, KEMThreeVector fN2, double fU, /* potential */
        double fNRot, int fNumDiscA, int fNumDiscB)
{
    ...
    for( int i=0; i<fNumDiscA; i++ ) {
        ...
        for( int j=0; j<fNumDiscB; j++ ) {
            ...
            rectangle->SetA( a[i] );
            // set P0
            KPosition p0New((fP0[0] + dA*fN1[0] + dB*fN2[0]), (fP0[1] + dA*fN1[1] + dB*fN2[1]), (fP0[2] + dA*fN1[2] + dB*fN2[2]));
 ...
 */
void AddWire(KSurfaceContainer &fContainer, int &fGroup, int &fChDen,
        KPosition fPA, KPosition fPB, double fD /*diameter*/ , double fU /*potential*/, int fNRot, int fNumDisc)
{
    (void)fNRot;
    fChDen += fNumDisc;
...
void AddBoundary(KSurfaceContainer &fContainer, int &fGroup, int &fChDen, double fA, double fB,
        KEMThreeVector fP0, KEMThreeVector fN1, KEMThreeVector fN2, double fEpsRAbove, double fEpsRBelow,
        double fNRot, int fNumDiscA, int fNumDiscB)
{
    (void)fNRot;
    fChDen += (fNumDiscA * fNumDiscB);
    ...
    if( fNumDiscA==0 || fNumDiscB==0 ) {
        ...
        //rectangle->SetNormalBoundaryFlux( fEpsRBelow / fEpsRAbove ); // TODO: Check!!!!
        rectangle->SetNormalBoundaryFlux(0, fEpsRAbove );
        rectangle->SetNormalBoundaryFlux(1, fEpsRBelow );
    ...
    for( int i=0; i<fNumDiscA; i++ ) {
        ...
        for( int j=0; j<fNumDiscB; j++ ) {
            ...
            //rectangle->SetNormalBoundaryFlux( fEpsRBelow / fEpsRAbove ); // TODO: Check!!!!
            rectangle->SetNormalBoundaryFlux(1, fEpsRAbove );
            rectangle->SetNormalBoundaryFlux(0, fEpsRBelow );
...
// ----------------------------------------------------------------------------
    ...
    (void) fFixedPoints;
    KEMThreeVector gLocation(0., 0., -0.25);
    int gSteps(5000);
...
// ----------------------------------------------------------------------------
    ...
    if (!fConnectWireEndPoints) fWireConnectDistX = 0.e0;
...
// ----------------------------------------------------------------------------
    ...
    // Start- and endpoints:
    KEMThreeVector fWireAnodePA(fWireLengthX,fRunAbsY,fLayerDistanceZ);
    KEMThreeVector fWireAnodePB(-fWireLengthX,fRunAbsY,fLayerDistanceZ);
    KEMThreeVector fWireCathodePA(fWireLengthX,fRunAbsY,-fLayerDistanceZ);
    KEMThreeVector fWireCathodePB(-fWireLengthX,fRunAbsY,-fLayerDistanceZ);

    // End points of connecting wires:
    KEMThreeVector fWireConnectPA;
    KEMThreeVector fWireConnectPB;

    // Normal vectors:
    KEMThreeVector fNx(1.,0.,0.);
    KEMThreeVector fNy(0.,1.,0.);
    KEMThreeVector fNz(0.,0.,1.);

    // Common P0 vector for rectangles:
    KEMThreeVector fP0(0.,0.,0.);
...
// ----------------------------------------------------------------------------
    ...
    if( fUseWires ) {
        ...
        for( int i=1; i<=fWiresPerLayer; i++ ) {
            ...
            if (fAnode) {
                ...
                fWireAnodePA.SetY(fRunAbsY); fWireAnodePB.SetY(fRunAbsY);
                AddWire(surfaceContainer, fGroupIndex, fChDensities, fWireAnodePA, fWireAnodePB, fWireDiameter, fAnodeU, 1, fWireDisc);
                ...
                if (fConnectWireEndPoints) {
                    ...
                    AddWire(surfaceContainer, fGroupIndex, fChDensities, fWireConnectPA, fWireConnectPB, fWireDiameter, fAnodeU, 1, fWireConnectDisc);
                    if (i<fWiresPerLayer) {
            ...
            if( fCathode ) {
                ...
                // Positive y-direction, y > 0
                fWireCathodePA.SetY(fRunAbsY); fWireCathodePB.SetY(fRunAbsY);
                AddWire(surfaceContainer, fGroupIndex, fChDensities, fWireCathodePA, fWireCathodePB, fWireDiameter, fCathodeU, 1, fWireDisc);
                ...
                if (fConnectWireEndPoints) {
                    ...
                    fWireConnectPA.SetComponents(fWireLengthX+fWireConnectDistX, fRunAbsY, -fLayerDistanceZ);
                    fWireConnectPB.SetComponents(fWireLengthX+fWireConnectDistX, fRunAbsY-fStepY, -fLayerDistanceZ);
                    AddWire(surfaceContainer, fGroupIndex, fChDensities, fWireConnectPA, fWireConnectPB, fWireDiameter, fCathodeU, 1, fWireConnectDisc);
                    if (i<fWiresPerLayer) {
                        // to ensure a quadratic area, the following step would be too much
                        fWireConnectPB.SetComponents(fWireLengthX+fWireConnectDistX, fRunAbsY+fStepY, -fLayerDistanceZ);
                        AddWire(surfaceContainer, fGroupIndex, fChDensities, fWireConnectPA, fWireConnectPB, fWireDiameter, fCathodeU, 1, fWireConnectDisc);
    ...
    if( !fUseWires ) {
        ...
        if (fAnode) {
            ...
            AddRect(surfaceContainer, fGroupIndex, fChDensities, fWireLengthX, fWireLengthX, fP0, fNx, fNy, fAnodeU, 1, fRectElectrodeDisc, fRectElectrodeDisc);
        ...
        if (fCathode) {
            fP0.SetComponents(-fWireLengthX, -fWireLengthX, -fLayerDistanceZ);
            AddRect(surfaceContainer, fGroupIndex, fChDensities, fWireLengthX, fWireLengthX, fP0, fNx, fNy, fCathodeU, 1, fRectElectrodeDisc, fRectElectrodeDisc);
...
// ----------------------------------------------------------------------------
    ...
    if( fBoxXY ) {
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fXY123567LengthX, fXY134578WidthY, fP0, fNx, fNy, fBoxU, 1,
                fRectDiscXY1357, fRectDiscXY1357);

        /* No. 2 */ fP0.SetComponents(-fBoxLengthX,-fXY269WidthY/2.e0,-fBoxHeightZ);
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fXY123567LengthX, fXY269WidthY, fP0, fNx, fNy, fBoxU, 1,
                fRectDiscXY26, fRectDiscXY26);
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fXY489LengthX, fXY134578WidthY, fP0, fNx, fNy, fBoxU, 1,
                fRectDiscXY48, fRectDiscXY48);
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fXY489LengthX, fXY269WidthY, fP0, fNx, fNy, fBoxU, 1,
                fRectDiscXY9, fRectDiscXY9);
...
// ----------------------------------------------------------------------------
    ...
    if( fBoundary ) {
        ...
        AddBoundary(surfaceContainer, fGroupIndex, fChDensities, fXY123567LengthX, fXY134578WidthY, fP0, fNx, fNy, fGXeEpsR, fLXeEpsR, 1,
                fBoundaryDiscXY1357, fBoundaryDiscXY1357);

        /* No. 2 */ fP0.SetComponents(-fBoxLengthX,-fXY269WidthY/2.e0,fBoundaryHeightZ);
        AddBoundary(surfaceContainer, fGroupIndex, fChDensities, fXY123567LengthX, fXY269WidthY, fP0, fNx, fNy, fGXeEpsR, fLXeEpsR, 1,
                fBoundaryDiscXY26, fBoundaryDiscXY26);
        ...
        AddBoundary(surfaceContainer, fGroupIndex, fChDensities, fXY489LengthX, fXY134578WidthY, fP0, fNx, fNy, fGXeEpsR, fLXeEpsR, 1,
                fBoundaryDiscXY48, fBoundaryDiscXY48);
...
// Subdividing rectangle no. 9 into 4 additional rectangles:
        ...
        AddBoundary(surfaceContainer, fGroupIndex, fChDensities, fXY489LengthX/2., fXY269WidthY/2., fP0, fNx, fNy, fGXeEpsR, fLXeEpsR, 1,
                fBoundaryDiscXY9, fBoundaryDiscXY9);
...
// ----------------------------------------------------------------------------
    ...
    if( fBoxYZ ) {
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fYZ1346WidthY, fYZ123HeightZ, fP0, fNy, fNz, fBoxU, 1,
                fRectDiscYZ13, fRectDiscYZ13);

        /* No. 2 */ fP0.SetComponents(-fBoxLengthX,-fYZ25WidthY/2.e0,-fLayerDistanceZ);
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fYZ25WidthY, fYZ123HeightZ, fP0, fNy, fNz, fBoxU, 1,
                fRectDiscYZ2, fRectDiscYZ2);
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fYZ1346WidthY, fYZ456HeightZ, fP0, fNy, fNz, fBoxU, 1,
                fRectDiscYZ46, fRectDiscYZ46);

        /* No. 5 */ fP0.SetComponents(-fBoxLengthX,-fYZ25WidthY/2.e0,-fBoxHeightZ);
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fYZ25WidthY, fYZ456HeightZ, fP0, fNy, fNz, fBoxU, 1,
                fRectDiscYZ5, fRectDiscYZ5);
...
// ----------------------------------------------------------------------------
    ...
    if( fBoxZX ) {
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fZX123HeightZ, fZX1346LengthX, fP0, fNz, fNx, fBoxU, 1,
                fRectDiscZX13, fRectDiscZX13);

        /* No. 2 */ fP0.SetComponents(-fZX25LengthX/2.e0,-fBoxWidthY,-fLayerDistanceZ);
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fZX123HeightZ, fZX25LengthX, fP0, fNz, fNx, fBoxU, 1,
                fRectDiscZX2, fRectDiscZX2);
        ...
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fZX456HeightZ, fZX1346LengthX, fP0, fNz, fNx, fBoxU, 1,
                fRectDiscZX46, fRectDiscZX46);

        /* No. 5 */ fP0.SetComponents(-fZX25LengthX/2.e0,-fBoxWidthY,-fBoxHeightZ);
        AddRect(surfaceContainer, fGroupIndex, fChDensities, fZX456HeightZ, fZX25LengthX, fP0, fNz, fNx, fBoxU, 1,
                fRectDiscZX5, fRectDiscZX5);
...
// ----------------------------------------------------------------------------
    ...
    KEMThreeVector fTimeChDen2(0.,0.,0.);
    ...
    fTimeChDenStart = clock();
        ...
        KOpenCLElectrostaticBoundaryIntegrator* fOCLIntegrator2 = new KOpenCLElectrostaticBoundaryIntegrator{
...
// ----------------------------------------------------------------------------
    ...
    std::cout << fTimeChDen2.Z() << " hours , " << fTimeChDen2.Y() << " minutes , " << fTimeChDen2.X() << " seconds." << std::endl <<       std::endl;
...
// ----------------------------------------------------------------------------
    ...
    KEMThreeVector fTimeField2(0., 0., 0.);

    fTimeFieldStart = clock();


    KEMThreeVector gEField(0.,0.,0.);
    double gPotential(0.);
    ...
    double Er(0.);
    std::pair<KEMThreeVector,double> result;
...
// ----------------------------------------------------------------------------
    ...
    std::cout << fTimeField2.Z() << " hours , " << fTimeField2.Y() << " minutes , " << fTimeField2.X() << " seconds." << std::endl;
