...
void kfm_matrix_set(const gsl_matrix* src, gsl_matrix* dest)
{
...
#ifdef KFMMATH_DEBUG
            kfmout << "kfm_matrix_svd: warning, SVD failed to converge within " << n_max_iter << " iterations, "
                   << kfmendl;
#endif
            break;
        }
...
void kfm_matrix_euler_angles(const kfm_matrix* R, double& alpha, double& beta, double& gamma, double& tol)
{
    ...
    if (R->size1 == 3 && R->size2 == 3) {
        ...
        if (!isDegenerate) {
            ...
            alpha = std::atan2((-1.0 * kfm_matrix_get(R, 2, 1)) / std::sin(beta),
                               (kfm_matrix_get(R, 2, 0)) / std::sin(beta));
            gamma = std::atan2((-1.0 * kfm_matrix_get(R, 1, 2)) / std::sin(beta),
                               -1 * (kfm_matrix_get(R, 0, 2)) / std::sin(beta));
        }
        else {
...
void kfm_matrix_householder_bidiagonalize(const kfm_matrix* A, kfm_matrix* P, kfm_matrix* J, kfm_matrix* Q)
{
    ...
    if (A->size1 == A->size2) {
        if (P->size1 != n || P->size2 != n || J->size1 != n || J->size2 != n || Q->size1 != n || Q->size2 != n) {
            kfmout << "kfm_matrix_householder_bidiagonalize: error, matrices A, P, J, or Q have different sizes."
                   << kfmendl;
            kfmout << "matrix A is " << A->size1 << " by " << A->size2 << "." << kfmendl;
...
void kfm_matrix_householder(kfm_matrix* H, const kfm_vector* w)
{
