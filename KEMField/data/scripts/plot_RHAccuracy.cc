...
std::vector<std::string> Tokenize(std::string separators,
				  std::string input)
{
...
void plot_RHAccuracy(std::string ws_name)
{
  ...
  if (!in) { std::cout<<"No file!"<<std::endl; return 1; }
  ...
  std::vector<std::vector<Double_t> > accuracyMatrix(nGroups,
						     std::vector<Double_t>(nIterations,0));
  ...
  if (nIterations>1)
    iterationsPerSec = (iterationArray[nIterations-1]-iterationArray[nIterations-2])/(timeArray[nIterations-1]-timeArray[nIterations-2]);
  ...
  for (Int_t j=nGroups-1;j>=0;j--)
  {
    ...
    if (j==0)
    {
      ...
      {
	g->Fit("expo","","",
	       iterationArray[nIterations-100],
	       iterationArray[nIterations-1]);
