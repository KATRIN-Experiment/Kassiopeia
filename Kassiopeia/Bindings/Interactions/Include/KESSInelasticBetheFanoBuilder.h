...
template<> inline bool KESSInelasticBetheFanoBuilder::AddAttribute(KContainer* aContainer)
{
    ...
    if (aContainer->GetName() == "PhotoAbsorption") {
        if (aContainer->AsReference<bool>()) {
            ...
            intmsg_debug(
                "KESSInelasticBetheFanoBuilder::AddAttribute: added a PhotoAbsorption calculator to KESSInelasticBetheFano"
                << eom);
        }
        else {
            intmsg_debug(
                "KESSInelasticBetheFanoBuilder::AddAttribute: PhotoAbsorption calculator is not added to KESSInelasticBetheFano"
                << eom);
        }
        return true;
    }
    if (aContainer->GetName() == "AugerRelaxation") {
        if (aContainer->AsReference<bool>()) {
            ...
            intmsg_debug(
                "KESSInelasticBetheFanoBuilder::AddAttribute: added an AugerRelaxation calculator to KESSInelasticBetheFano"
                << eom);
        }
        else {
            intmsg_debug(
                "KESSInelasticBetheFanoBuilder::AddAttribute: AugerRelaxation calculator is not added to KESSInelasticBetheFano"
                << eom);
        }
        return true;
    }
