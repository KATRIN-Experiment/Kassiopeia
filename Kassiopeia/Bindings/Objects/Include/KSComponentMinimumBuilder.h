...
inline KSComponent* BuildOutputMinimum(KSComponent* aComponent)
{
...
template<> inline bool KSComponentMinimumBuilder::Begin()
{
    fObject = new KSComponentMinimumData;
    return true;
}

template<> inline bool KSComponentMinimumBuilder::AddAttribute(KContainer* aContainer)
{
    ...
    if (aContainer->GetName() == "component") {
        objctmsg(eWarning)
            << "deprecated warning in KSComponentMinimumBuilder: Please use the attribute <parent> instead <component>"
            << eom;
...
template<> inline bool KSComponentMinimumBuilder::End()
{
    ...
    if (fObject->fGroupName.empty() == false) {
        ...
        if (tParentComponent == nullptr) {
            objctmsg(eError) << "component minimum builder could not find component <" << fObject->fParentName
