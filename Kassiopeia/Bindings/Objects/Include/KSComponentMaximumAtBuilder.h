...
inline KSComponent* BuildOutputMaximumAt(KSComponent* aComponent, KSComponent* aSource)
{

#define BUILD_OUTPUT(xVALUE, xSOURCE)                                                                                  \
    if ((aComponent->Is<xVALUE>() == true) && (aSource->Is<xSOURCE>() == true)) {                                      \
        return new KSComponentMaximumAt<xVALUE, xSOURCE>(aComponent,                                                   \
...
template<> inline bool KSComponentMaximumAtBuilder::Begin()
{
    fObject = new KSComponentMaximumAtData;
    return true;
}

template<> inline bool KSComponentMaximumAtBuilder::AddAttribute(KContainer* aContainer)
{
    ...
    if (aContainer->GetName() == "component") {
        objctmsg(eWarning)
            << "deprecated warning in KSComponentMaximumAtBuilder: Please use the attribute <parent> instead <component>"
            << eom;
...
template<> inline bool KSComponentMaximumAtBuilder::End()
{
    ...
    if (fObject->fGroupName.empty() == false) {
        ...
        if (tParentComponent == nullptr) {
            objctmsg(eError) << "component maximum_at builder could not find component <" << fObject->fParentName
                             << "> in group <" << fObject->fGroupName << ">" << eom;
            return false;
        }
        if (tSourceComponent == nullptr) {
            objctmsg(eError) << "component maximum_at builder could not find component <" << fObject->fSourceName
                             << "> in group <" << fObject->fGroupName << ">" << eom;
            return false;
        }
    }
