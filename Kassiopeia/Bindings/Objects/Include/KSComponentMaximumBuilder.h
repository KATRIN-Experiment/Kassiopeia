...
template<> inline bool KSComponentMaximumBuilder::Begin()
{
...
template<> inline bool KSComponentMaximumBuilder::End()
{
    ...
    if (fObject->fGroupName.empty() == false) {
        ...
        if (tParentComponent == nullptr) {
            objctmsg(eError) << "component maximum builder could not find component <" << fObject->fParentName
