...
void KSWriteHDF5::Data::MakeDataSets(KSComponent* aComponent)
{
    ...
    fStructure =
        H5Dcreate2(fFile, tStructureName, H5T_NATIVE_CHAR, tStructureSpace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    fStructure
...
{
    wtrmsg_debug("HDF5 writer <" << GetName() << "> is filling a run" << eom);
...
{
    wtrmsg_debug("HDF5 writer <" << GetName() << "> is filling an event" << eom);
...
{
    wtrmsg_debug("HDF5 writer <" << GetName() << "> is filling a track" << eom);
...
void KSWriteHDF5::ExecuteStep()
{
    if (fStepIterationIndex % fStepIteration != 0) {
        wtrmsg_debug("HDF5 writer <" << GetName() << "> is skipping a step because of step iteration value <"
                                     << fStepIteration << ">" << eom);
        fStepIterationIndex++;
        return;
    }
    ...
    if (fStepComponent == true) {
        wtrmsg_debug("HDF5 writer <" << GetName() << "> is filling a step" << eom);
