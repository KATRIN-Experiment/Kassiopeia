...
void KSWriteROOT::AddRunWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    ...
    for (int tIndex = 0; tIndex < fRunWriteConditions.End(); tIndex++) {
        if (aWriteCondition == fRunWriteConditions.ElementAt(tIndex)) {
            wtrmsg_debug("<" << GetName() << "> attempted to add run write condition <" << aWriteCondition->GetName()
    ...
    if (fRunWriteConditions.AddElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not add run write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::RemoveRunWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    if (fRunWriteConditions.RemoveElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not remove run write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::AddEventWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    ...
    for (int tIndex = 0; tIndex < fEventWriteConditions.End(); tIndex++) {
        if (aWriteCondition == fEventWriteConditions.ElementAt(tIndex)) {
            wtrmsg_debug("<" << GetName() << "> attempted to add event write condition <" << aWriteCondition->GetName()
                             << "> which is already present." << eom);
            return;
        }
    }
    ...
    if (fEventWriteConditions.AddElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not add event write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::RemoveEventWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    if (fEventWriteConditions.RemoveElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not remove event write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::AddTrackWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    ...
    for (int tIndex = 0; tIndex < fTrackWriteConditions.End(); tIndex++) {
        if (aWriteCondition == fTrackWriteConditions.ElementAt(tIndex)) {
            wtrmsg_debug("<" << GetName() << "> attempted to add track write condition <" << aWriteCondition->GetName()
                             << "> which is already present." << eom);
            return;
        }
    }
    ...
    if (fTrackWriteConditions.AddElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not add track write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::RemoveTrackWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    if (fTrackWriteConditions.RemoveElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not remove track write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::AddStepWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    ...
    for (int tIndex = 0; tIndex < fStepWriteConditions.End(); tIndex++) {
        if (aWriteCondition == fStepWriteConditions.ElementAt(tIndex)) {
            wtrmsg_debug("<" << GetName() << "> attempted to add step write condition <" << aWriteCondition->GetName()
                             << "> which is already present." << eom);
            return;
        }
    }
    ...
    if (fStepWriteConditions.AddElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not add step write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::RemoveStepWriteCondition(KSWriteROOTCondition* aWriteCondition)
{
    if (fStepWriteConditions.RemoveElement(aWriteCondition) == -1) {
        wtrmsg(eError) << "<" << GetName() << "> could not remove step write condition <" << aWriteCondition->GetName()
                       << ">" << eom;
        return;
    }
...
void KSWriteROOT::ExecuteStep()
{
    if (fStepIterationIndex % fStepIteration != 0) {
        wtrmsg_debug("ROOT writer <" << GetName() << "> is skipping a step because of step iteration value <"
                                     << fStepIteration << ">" << eom);
        fStepIterationIndex++;
        return;
    }
...
STATICINT sKSWriteROOTDict =
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddRunComponent, &KSWriteROOT::RemoveRunComponent,
                                          "add_run_output", "remove_run_output") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddEventComponent, &KSWriteROOT::RemoveEventComponent,
                                          "add_event_output", "remove_event_output") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddTrackComponent, &KSWriteROOT::RemoveTrackComponent,
                                          "add_track_output", "remove_track_output") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddStepComponent, &KSWriteROOT::RemoveStepComponent,
                                          "add_step_output", "remove_step_output") +

    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddRunWriteCondition, &KSWriteROOT::RemoveRunWriteCondition,
                                          "add_run_write_condition", "remove_run_write_condition") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddEventWriteCondition, &KSWriteROOT::RemoveEventWriteCondition,
                                          "add_event_write_condition", "remove_event_write_condition") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddTrackWriteCondition, &KSWriteROOT::RemoveTrackWriteCondition,
                                          "add_track_write_condition", "remove_track_write_condition") +
    KSDictionary<KSWriteROOT>::AddCommand(&KSWriteROOT::AddStepWriteCondition, &KSWriteROOT::RemoveStepWriteCondition,
                                          "add_step_write_condition", "remove_step_write_condition");
}  // namespace Kassiopeia
