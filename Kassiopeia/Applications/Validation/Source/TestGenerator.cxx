...
int main(int anArgc, char** anArgv)
{
    ...
    if (anArgc <= 1) {
        ...
        mainmsg << ret;
        mainmsg << "available generator components found in " << CONFIG_DEFAULT_DIR << "/Validation/TestGenerator.xml"
                << ret;
    ...
    TH3D tGeneratorLocationHistogram("generator_location_histogram",
                                     "generator_location_histogram",
                                     100,
                                     -1.1,
                                     1.1,
                                     100,
    ...
    TH3D tParticlePositionHistogram("particle_position_histogram",
                                    "particle_position_histogram",
                                    100,
                                    -1.1,
                                    1.1,
                                    100,
    ...
    TH3D tParticleDirectionHistogram("particle_direction_histogram",
                                     "particle_direction_histogram",
                                     100,
                                     -1.1,
                                     1.1,
                                     100,
    ...
    TH1D tParticleDirectionThetaHistogram("particle_direction_theta_histogram",
                                          "particle_direction_theta_histogram",
                                          90,
                                          0.,
                                          180.);

    TCanvas tParticleDirectionPhiCanvas("particle_direction_phi_canvas", "Particle Direction Azimuthal Angle");
    TH1D tParticleDirectionPhiHistogram("particle_direction_phi_histogram",
                                        "particle_direction_phi_histogram",
                                        180,
                                        0.,
                                        360.);
    ...
    for (unsigned int tIndex = 0; tIndex < tCount; tIndex++) {
        ...
        tGeneratorLocationHistogram.Fill(tEvent.GeneratorLocation().X(),
                                         tEvent.GeneratorLocation().Y(),
                                         tEvent.GeneratorLocation().Z());
        ...
        for (tParticleIt = tEvent.ParticleQueue().begin(); tParticleIt != tEvent.ParticleQueue().end(); tParticleIt++) {
            ...
            tParticlePositionHistogram.Fill(tParticle->GetPosition().X(),
                                            tParticle->GetPosition().Y(),
                                            tParticle->GetPosition().Z());
            tParticleDirectionHistogram.Fill(tParticle->GetMomentum().Unit().X(),
                                             tParticle->GetMomentum().Unit().Y(),
                                             tParticle->GetMomentum().Unit().Z());
            tParticleDirectionThetaHistogram.Fill((180. / katrin::KConst::Pi()) *
                                                  tParticle->GetMomentum().PolarAngle());
            tParticleDirectionPhiHistogram.Fill((180. / katrin::KConst::Pi()) *
                                                tParticle->GetMomentum().AzimuthalAngle());
