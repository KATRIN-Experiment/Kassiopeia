...
int main( int argc, char** argv )
{
  ...
  if( argc <= 2 )
  {
    ...
    mainmsg << ret;
    mainmsg << "default field components found in " << CONFIG_DEFAULT_DIR << "/Validation/TestPotentialmap.xml" << ret;
    mainmsg << eom;
    return -1;
  }
  ...
  for (unsigned int i=0;i<tCount;i++)
  {
    ...
    {
      std::cout<<"  Electric Potential Diff:   "<<(tElectricPotentialDirect-tElectricPotentialFromMap)<<std::endl;
  ...
  if (tRootElectricFieldDirect && tRootElectricFieldFromMap)
  {
    ...
    if (tPlotPotential)
    {
      ...
      if (fieldCanvas)
      {
        ...
        std::cout<<"Computing potential differences on a "<<N_x<<" by "<<N_z<<" grid ("<<countermax<<" points)"<<std::endl;
    ...
    if (tPlotField)
    {
      ...
      if (fieldCanvas)
      {
        ...
        std::cout<<"Computing field differences on a "<<N_x<<" by "<<N_z<<" grid ("<<countermax<<" points)"<<std::endl;
