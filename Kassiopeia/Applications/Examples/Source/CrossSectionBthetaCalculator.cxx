...
int main(int argc, char** argv)
{
    if (argc < 9) {
        cout
            ...
            << endl;
        // ./CrossSectionBthetaCalculator ~/Work/kasper/install/config/Kassiopeia/Examples/TestProject8Simulation.xml -1.0 1.0 -1.0 1.0 0.0 1000 field_electromagnet
        // ./SimpleMagneticFieldCalculator ~/Work/kasper/install/config/Kassiopeia/Examples/TestProject8Simulation.xml 0.6143 0.0972 0.0 field_electromagnet
        exit(-1);
    }
...
//#endif
    ...
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j <= n; j++) {
            ...
            magnetic_field_magnitudes[i][j] = tMagneticField.Cross(tPosition).Magnitude() / tPosition.Magnitude();
    ...
    TH2* h2 = new TH2D("h2", "Central Cross-Section B_theta", n + 1, x_min, x_max, n + 1, y_min, y_max);
