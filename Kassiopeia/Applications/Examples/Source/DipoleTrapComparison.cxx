...
int main()
{
    ...
    for (exact_tRunReader = 0; exact_tRunReader <= exact_tRunReader.GetLastRunIndex(); exact_tRunReader++) {
        for (exact_tEventReader = exact_tRunReader.GetFirstEventIndex();
             exact_tEventReader <= exact_tRunReader.GetLastEventIndex();
             exact_tEventReader++) {
            for (exact_tTrackReader = exact_tEventReader.GetFirstTrackIndex();
                 exact_tTrackReader <= exact_tEventReader.GetLastTrackIndex();
                 exact_tTrackReader++) {
                for (exact_tStepReader = exact_tTrackReader.GetFirstStepIndex();
                     exact_tStepReader <= exact_tTrackReader.GetLastStepIndex();
                     exact_tStepReader++) {
    ...
    for (adiabatic_tRunReader = 0; adiabatic_tRunReader <= adiabatic_tRunReader.GetLastRunIndex();
         adiabatic_tRunReader++) {
        for (adiabatic_tEventReader = adiabatic_tRunReader.GetFirstEventIndex();
             adiabatic_tEventReader <= adiabatic_tRunReader.GetLastEventIndex();
             adiabatic_tEventReader++) {
            for (adiabatic_tTrackReader = adiabatic_tEventReader.GetFirstTrackIndex();
                 adiabatic_tTrackReader <= adiabatic_tEventReader.GetLastTrackIndex();
                 adiabatic_tTrackReader++) {
                for (adiabatic_tStepReader = adiabatic_tTrackReader.GetFirstStepIndex();
                     adiabatic_tStepReader <= adiabatic_tTrackReader.GetLastStepIndex();
                     adiabatic_tStepReader++) {
    ...
    for (i = 0; i < 3000; i++) {
        ...
        momentum_diff_list->push_back(((*adiabatic_momentum_z_list)[i] - (*exact_momentum_z_list)[i]) /
                                      (*exact_momentum_z_list)[0]);
