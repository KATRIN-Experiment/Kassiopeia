...
void KESSPhotoAbsorbtion::ReadIonisationPDF(std::string data_filename)
{
    ...
    if (ionizationFile == nullptr) {
        intmsg(eError) << "KESSPhotoAbsorbtion::ReadIonisationPDF" << ret << "FILE "
                       << myPathToTable + "/" + data_filename << " NOT FOUND CHECK IN IO CONIGFILE FOR DATA DIRECTORY"
                       << eom;
    }
...
void KESSPhotoAbsorbtion::CreateSecondary(const double secondaryEnergy, const KSParticle& aFinalParticle,
...
unsigned int KESSPhotoAbsorbtion::IonizeShell(const double& lostEnergy_eV, const KSParticle& aFinalParticle,
                                              KSParticleQueue& aQueue)
{
    ...
    if (lostEnergy_eV > eK) {
        ...
        if (ionizedShell == 3) {
            ...
            this->CreateSecondary(secondaryEnergy, aFinalParticle, aQueue);
        }
...
unsigned int KESSPhotoAbsorbtion::FindIonizedShell(double lostEnergy_eV)
{
    ...
    intmsg(eWarning) << "KESSPhotoAbsorbtion::FindIonizedShell" << ret
                     << "Probability mismatch in KESSPhotoAbsorbtion at energy: " << lostEnergy_eV << eom;
    return 3;
}
