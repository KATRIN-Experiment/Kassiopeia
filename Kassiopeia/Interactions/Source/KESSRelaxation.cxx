...
KESSRelaxation::KESSRelaxation() : fSiliconBandGap(0.) {}

void KESSRelaxation::RelaxAtom(unsigned int vacantShell, const KSParticle& aFinalParticle, KSParticleQueue& aQueue)
{
...
void KESSRelaxation::RelaxL23Shell(const KSParticle& aFinalParticle, KSParticleQueue& aQueue)
{

    CreateMMVacancies(23, aFinalParticle, aQueue);
}

void KESSRelaxation::RelaxL1Shell(const KSParticle& aFinalParticle, KSParticleQueue& aQueue)
{
    ...
    if (random < 0.975) {  //			49.5eV
                           //			L23 vacancy

        double holeEnergy = this->GetBindingEnergy(3) * KRandom::GetInstance().Uniform();
        augerEnergy = eL1 - eL2 - holeEnergy - fSiliconBandGap;

        CreateAugerElectron(augerEnergy, aFinalParticle, aQueue);
        this->RelaxL23Shell(aFinalParticle, aQueue);
    }
    else if (random >= 0.975 && random < 1) {
        //			132eV
        //			M,M vacancy
        CreateMMVacancies(21, aFinalParticle, aQueue);
    }
    else {
...
void KESSRelaxation::RelaxKShell(const KSParticle& aFinalParticle, KSParticleQueue& aQueue)
{
    ...
    if (random <= 0) {
        //KMM
        CreateMMVacancies(1, aFinalParticle, aQueue);
        return;
    }
    else {
...
void KESSRelaxation::CreateMMVacancies(unsigned int fromThisShell, const KSParticle& aFinalParticle,
                                       KSParticleQueue& aQueue)
{
    ...
    this->CreateAugerElectron(augerEnergy, aFinalParticle, aQueue);
}
...
void KESSRelaxation::CreateAugerElectron(const double& augerEnergy_eV, const KSParticle& aFinalParticle,
                                         KSParticleQueue& aQueue)
{
