...
void KSGenPositionMeshSurfaceRandom::VisitExtendedSurface(
    KGeoBag::KGExtendedSurface<KGeoBag::KGMesh>* anExtendedSurface)
{
...
void KSGenPositionMeshSurfaceRandom::Dice(KSParticleQueue* aPrimaries)
{
    for (auto& aPrimarie : *aPrimaries) {
        ...
        if (auto* tMeshRectangle = dynamic_cast<KGeoBag::KGMeshRectangle*>(*tElementIt)) {
            KThreeVector tInternalRandomPosition =
                tMeshRectangle->GetP0() +
                KRandom::GetInstance().Uniform() * tMeshRectangle->GetA() * tMeshRectangle->GetN1() +
                KRandom::GetInstance().Uniform() * tMeshRectangle->GetB() * tMeshRectangle->GetN2();

            KThreeVector tExternalRandomPosition =
        ...
        if (auto* tMeshWire = dynamic_cast<KGeoBag::KGMeshWire*>(*tElementIt)) {
            ...
            KThreeVector tInternalRandomPosition =
                tStart + KRandom::GetInstance().Uniform() * tConnection + tRadius * (cos(tPhi) * tU + sin(tPhi) * tV);
        ...
        genmsg(eError) << "Could not dice a position! Only rectangles, triangles and wires are acceptable elements. "
                       << eom;
    }
}
