 ...
 */
template<class Float> using KMathIntegratorThreaded = KMathIntegrator<Float, policies::ThreadedSumming>;
template<class Float> using KMathIntegrator2DThreaded = KMathIntegrator2D<Float, policies::ThreadedSumming>;
...
struct ThreadedKahanSumming : KahanSumming
{
    ...
    static Float SumSamplingPoints(uint32_t n, Float xStart, Float del, XIntegrandType&& integrand)
    {
        return ThreadedSumming::SumSamplingPoints<Float, XIntegrandType, KMathKahanSum<Float>>(
            n,
            xStart,
            del,
