 ...
 */
template<int NDIM = 1> class KMathShepardInterpolator
{
  public:
...
template<int NDIM> inline void KMathShepardInterpolator<NDIM>::AddValue(const std::vector<double>& point, double value)
{
...
template<int NDIM> inline void KMathShepardInterpolator<NDIM>::AddValue(double point, double value)
{
    assert(NDIM == 1);
    AddValue(&point, value);
}

template<int NDIM> inline double KMathShepardInterpolator<NDIM>::Calculate(const std::vector<double>& point) const
{
...
template<int NDIM> inline double KMathShepardInterpolator<NDIM>::Calculate(double point) const
{
    assert(NDIM == 1);
    return Interpolate(&point);
}

template<int NDIM> inline double KMathShepardInterpolator<NDIM>::Interpolate(const double* pt) const
{
...
template<int NDIM> inline double KMathShepardInterpolator<NDIM>::Rad2(const double* p1, const double* p2)
{
    double diff, sum = 0.0;
