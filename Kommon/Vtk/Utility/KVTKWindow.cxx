...
void KVTKWindow::Render()
{
    ...
    if (fDisplayToggle == true) {
        ...
        vtkSmartPointer<vtkInteractorStyleTrackballCamera> tInteractorStyle =
...
void KVTKWindow::AddPainter(KPainter* aPainter)
{
    ...
    if (tPainter != nullptr) {
        ...
        utilmsg(eError) << "cannot add vtk painter <" << tPainter->GetName() << "> to vtk window <" << GetName() << ">"
                        << eom;
    }
    utilmsg(eError) << "cannot add non-vtk painter <" << aPainter->GetName() << "> to vtk window <" << GetName() << ">"
                    << eom;
    return;
}
void KVTKWindow::RemovePainter(KPainter* aPainter)
{
    ...
    if (tPainter != nullptr) {
        ...
        utilmsg(eError) << "cannot remove vtk painter <" << tPainter->GetName() << "> from vtk window <" << GetName()
                        << ">" << eom;
    }
    utilmsg(eError) << "cannot remove non-vtk painter <" << aPainter->GetName() << "> from vtk window <" << GetName()
                    << ">" << eom;
    return;
}
...
void KVTKWindow::UpdateData()
{
    ...
    tText << "position:    [ " << std::setprecision(3) << cameraPosition[0] << ", " << cameraPosition[1] << ", "
          << cameraPosition[2] << " ]" << '\n';
    tText << "view point:  [ " << std::setprecision(3) << cameraFocus[0] << ", " << cameraFocus[1] << ", "
          << cameraFocus[2] << " ]" << '\n';
