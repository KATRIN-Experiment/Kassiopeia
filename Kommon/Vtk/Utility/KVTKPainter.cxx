...
namespace katrin
{
    ...
    void KVTKPainter::SetWindow( KWindow* aWindow )
    {
        ...
        if( tWindow != NULL )
        {
            ...
            utilmsg( eError ) << "cannot use vtk window <" << tWindow->GetName() << "> with vtk painter <" << GetName() << ">" << eom;
        }
        utilmsg( eError ) << "cannot use non-vtk window <" << aWindow->GetName() << "> with vtk painter <" << GetName() << ">" << eom;
        return;
    }
