...
KMessage::KMessage(const string& aKey, const string& aDescription, const string& aPrefix, const string& aSuffix) :
    ...
    fShowShutdownMessage(KMessageTable::GetInstance().GetShowShutdownMessage()),
    fShowParserContext(KMessageTable::GetInstance().GetShowParserContext()),
...
{
    KMessageTable::GetInstance().GetStacktracePrinterCallback()(aStream);
}

void KMessage::ParserContext(std::ostream& aStream)
{
    auto callback = KMessageTable::GetInstance().GetParserContextPrinterCallback();
    if (callback) {
        aStream << Prefix() << TabIndent << "while ";
        callback(aStream);
    }
}
...
KMessageTable::KMessageTable() :
    ...
    fLogStream(nullptr),
    fVerbosityLevel(0),
    fStacktracePrinterCallback(KMessageTable::DefaultStacktracePrinter)
{}
...
std::function<void(std::ostream&)> KMessageTable::SetParserContextPrinterCallback(std::function<void(std::ostream&)> callback)
{
    auto oldCallback = fParserContextPrinterCallback;
    fParserContextPrinterCallback = callback;
    return oldCallback;
}

std::function<void(std::ostream&)> KMessageTable::GetParserContextPrinterCallback() const
{
    return fParserContextPrinterCallback;
}

void KMessageTable::RemoveParserContextPrinterCallback()
{
    fParserContextPrinterCallback = nullptr;
    return;
}


std::function<void(std::ostream&)> KMessageTable::SetStacktracePrinterCallback(std::function<void(std::ostream&)> callback)
{
    auto oldCallback = fStacktracePrinterCallback;
    fStacktracePrinterCallback = callback;
    return oldCallback;
}

std::function<void(std::ostream&)> KMessageTable::GetStacktracePrinterCallback() const
{
    return fStacktracePrinterCallback;
}
...
void KMessageTable::DefaultStacktracePrinter(std::ostream& aStream)
{
    ...
    for (size_t Index = 0; Index < FrameCount; Index++) {
        aStream << FrameSymbols[Index] << std::endl;
    }

    free(FrameSymbols);
}

auto terminate_handler = std::set_terminate([](){
    try
    {
    ...
    catch (const std::exception& err)
    {
        try
        {
            KMessage("-", "-", "", "") << eError << err.what() << eom;
        }
        catch (const KException& err)
        {
            // Ignoring exception by KMessage
        }
    }
    std::abort();
});
