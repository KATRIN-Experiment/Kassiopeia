...
void KROOTPad::AddPainter(KPainter* aPainter)
{
    ...
    utilmsg(eError) << "cannot add non-root painter <" << aPainter->GetName() << "> to root pad <" << GetName() << ">"
                    << eom;
    return;
}
void KROOTPad::RemovePainter(KPainter* aPainter)
{
    ...
    if (tPainter != nullptr) {
        ...
        utilmsg(eError) << "cannot remove root painter <" << tPainter->GetName() << "> from root pad <" << GetName()
                        << ">" << eom;
    }
    utilmsg(eError) << "cannot remove non-root painter <" << aPainter->GetName() << "> from root pad <" << GetName()
                    << ">" << eom;
    return;
}
