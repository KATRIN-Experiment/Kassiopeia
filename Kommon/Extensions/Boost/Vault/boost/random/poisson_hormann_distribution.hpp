/* boost random/poisson_hormann_distribution.hpp header file
 *
 ...
 *
 * $Id: poisson_hormann_distribution.hpp 71018 2011-04-05 21:27:52Z steven_watanabe $
 *
 */

#ifndef BOOST_RANDOM_POISSON_HORMANN_DISTRIBUTION_HPP
#define BOOST_RANDOM_POISSON_HORMANN_DISTRIBUTION_HPP
...
struct poisson_hormann_table {
    static RealType value[10];
};

template<class RealType>
RealType poisson_hormann_table<RealType>::value[10] = {
    0.0,
    0.0,
    0.69314718055994529,
    1.7917594692280550,
...
/**
 * An instantiation of the class template @c poisson_hormann_distribution is a
...
class poisson_hormann_distribution {
public:
    ...
    class param_type {
    public:
        typedef poisson_hormann_distribution distribution_type;
        /**
    ...
    /**
     * Constructs a @c poisson_hormann_distribution with the parameter @c mean.
     *
     * Requires: mean > 0
     */
    explicit poisson_hormann_distribution(RealType mean_arg = RealType(1))
      : _mean(mean_arg)
    {
        BOOST_ASSERT(_mean > 0);
        init();
    ...
    /**
     * Construct an @c poisson_hormann_distribution object from the
     * parameters.
     */
    explicit poisson_hormann_distribution(const param_type& parm)
      : _mean(parm.mean())
    {
        init();
    }
    ...
    {
        return poisson_hormann_distribution(parm)(urng);
    }
    ...
    friend std::basic_ostream<CharT,Traits>&
    operator<<(std::basic_ostream<CharT,Traits>& os,
               const poisson_hormann_distribution& pd)
    {
        os << pd.param();
        return os;
    }
    ...
    template<class CharT, class Traits>
    friend std::basic_istream<CharT,Traits>&
    operator>>(std::basic_istream<CharT,Traits>& is, poisson_hormann_distribution& pd)
    {
        pd.read(is);
        return is;
    }
    ...
    friend bool operator==(const poisson_hormann_distribution& lhs,
                           const poisson_hormann_distribution& rhs)
    {
        return lhs._mean == rhs._mean;
    }
...
private:
    ...
    static RealType log_factorial(IntType k)
    {
        BOOST_ASSERT(k >= 0);
        BOOST_ASSERT(k < 10);
        return detail::poisson_hormann_table<RealType>::value[k];
    }
...
using random::poisson_hormann_distribution;
...
#endif // BOOST_RANDOM_POISSON_HORMANN_DISTRIBUTION_HPP
