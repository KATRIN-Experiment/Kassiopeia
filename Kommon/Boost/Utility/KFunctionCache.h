 ...
 *
 */
template<size_t D = 1, typename IndexT = int32_t> class KFunctionCache
{
  public:
  ...
  public:
    ...
    {
        fCache.SetMaxSize(maxCacheSize);
    }

    void SetMethod(EInterpolationMethod method)
    {
        fMethod = method;
    }

    void ConfigureParameter(size_t iParam, double gridConstant, double centerValue = 0.0,
                            boost::optional<double> lowerBound = boost::none,
                            boost::optional<double> upperBound = boost::none);

    template<class FunctionT> void SetFunction(FunctionT& function);

    static size_t NumberOfDimensions()
    {
        return D;
    }
  ...
  protected:
    ...
    bool FirstCombination(const std::array<IndexT, D>& start, double lower, std::array<IndexT, D>& result) const;
    bool LastCombination(const std::array<IndexT, D>& start, double upper, std::array<IndexT, D>& result) const;
    ...
    void ParameterValues(const std::array<IndexT, D>& gridIndices, std::array<double, D>& resultValues) const;
...
template<std::size_t D, typename IndexT> inline double KFunctionCache<D, IndexT>::Get(const std::vector<double>& params)
{
...
template<std::size_t D, typename IndexT> inline double KFunctionCache<D, IndexT>::Get(const double* params)
{
    std::array<double, D> barray;
...
template<std::size_t D, typename IndexT> inline double KFunctionCache<D, IndexT>::Get(const double& param)
{
    std::array<double, D> barray;
    barray[0] = param;
    return Get(barray);
                                                 ...
                                                 double& resultDistance) const
{
    ...
    const double floatGridIndex =
        (paramValue - fParamConfigs[iParam].fCenterValue) / fParamConfigs[iParam].fGridConstant;
...
inline void KFunctionCache<D, IndexT>::GridIndices(const std::array<double, D>& paramValues,
                                                   std::array<IndexT, D>& resultIndices,
                                                   std::array<double, D>& resultDistances) const
{
...
template<std::size_t D, typename IndexT>
inline double KFunctionCache<D, IndexT>::ParameterValue(uint32_t iParam, IndexT gridIndex, double gridDist) const
{
    assert(iParam < D);
    double result =
        fParamConfigs[iParam].fCenterValue + fParamConfigs[iParam].fGridConstant * ((double) gridIndex + gridDist);
...
template<std::size_t D, typename IndexT>
inline bool KFunctionCache<D, IndexT>::FirstCombination(const std::array<IndexT, D>& start, double lower,
...
template<std::size_t D, typename IndexT>
inline bool KFunctionCache<D, IndexT>::LastCombination(const std::array<IndexT, D>& start, double upper,
                                                       std::array<IndexT, D>& result) const
{
    for (std::size_t i = 0; i < D; ++i) {
 ...
 * @return
 */
template<size_t D, typename IndexT> double KFunctionCache<D, IndexT>::Get(const std::array<double, D>& paramValues)
{
    ...
    const bool combinationsSet = (FirstCombination(centerGridIndices, 0 - g, startGridIndices) &&
    ...
    do {
        ...
        for (size_t j = 0; j < D; ++j) {
            ...
            if (fMethod == EInterpolationMethod::Spline) {
                ...
                switch (i) {
                    case -1:
                        polynomProduct *= 0.5 * boost::math::pow<3>(x - 1.0) * x * (2.0 * x + 1.0);
                        break;
                    case 0:
                        polynomProduct *= -0.5 * (x - 1.0) *
                                          (6.0 * boost::math::pow<4>(x) - 9.0 * boost::math::pow<3>(x) + 2.0 * x + 2.0);
                        break;
                    case 1:
                        polynomProduct *= 0.5 * x *
                                          (6.0 * boost::math::pow<4>(x) - 15.0 * boost::math::pow<3>(x) +
                                           9.0 * boost::math::pow<2>(x) + x + 1.0);
                        break;
                    case 2:
                        polynomProduct *= -0.5 * (x - 1.0) * boost::math::pow<3>(x) * (2.0 * x - 3);
                        break;
                    default:
