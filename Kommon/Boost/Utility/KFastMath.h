...
{
  public:
    template<typename XFloatType> static XFloatType Sqrt_Heron(XFloatType a, uint32_t nIterations = 5);

    template<typename XFloatType> static XFloatType Exp_Taylor(XFloatType x, int32_t order);

    template<int NIteratons, typename XFloatType> static XFloatType Exp_Taylor(XFloatType x);
    ...
    template<typename XFloatType> static XFloatType FastSqrt_Q3(XFloatType x, uint32_t n = 2);

    static double FastExp(double y);
};

template<typename XFloatType> inline XFloatType KFastMath::Sqrt_Heron(XFloatType a, uint32_t nIterations)
{
    //    if (a == 1.0)
    //        return a;
...
template<typename XFloatType> inline XFloatType KFastMath::FastSqrt_Q3(XFloatType x, uint32_t n)
{
...
template<typename XFloatType> inline XFloatType KFastMath::Exp_Taylor(XFloatType x, int32_t n)
{
...
template<int NIteratons, typename XFloatType> inline XFloatType KFastMath::Exp_Taylor(XFloatType x)
{
