...
template <class Space, class Surface>
void FollowRecursion(Space* space);
...
int main()
{
  ...
  FollowRecursion<KGExtendedSpace<KGMesh>,KGExtendedSurface<KGMesh> >(KGInterface::GetInstance()->As<KGMesh>()->fRoot);
...
void FollowRecursion(Space* space)
{
  ...
  for ( SurfaceIt surfaceIt = space->GetBoundarySurfaces()->begin(); surfaceIt != space->GetBoundarySurfaces()->end(); ++surfaceIt )
  {
    for (int i=0;i<level;i++) std::cout<<" ";
    std::cout<<" - "<<(*surfaceIt)->GetName()<<std::endl;
  }

  for ( SurfaceIt surfaceIt = space->GetSurroundedSurfaces()->begin(); surfaceIt != space->GetSurroundedSurfaces()->end(); ++surfaceIt )
  {
    for (int i=0;i<level;i++) std::cout<<" ";
    std::cout<<" - "<<(*surfaceIt)->GetName()<<std::endl;
  }

  for( SpaceIt spaceIt = space->GetSurroundedSpaces()->begin(); spaceIt != space->GetSurroundedSpaces()->end(); ++spaceIt )
  {
    level++;
