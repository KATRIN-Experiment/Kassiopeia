...
int main()
{
  ...
  for( KGSurfaceIt surfaceIt = placedCylinder->GetBoundarySurfaces()->begin(); surfaceIt != placedCylinder->GetBoundarySurfaces()->end(); ++surfaceIt )
  {
  ...
  for( KGMeshSurfaceIt meshSurfaceIt = meshedCylinder->GetBoundarySurfaces()->begin(); meshSurfaceIt != meshedCylinder->GetBoundarySurfaces()->end(); meshSurfaceIt++ )
  {
    Int_t i = 0;
    for( KGMeshElementCIt elementIt = (*meshSurfaceIt)->GetMeshElements()->begin(); elementIt != (*meshSurfaceIt)->GetMeshElements()->end(); ++elementIt )
    {
	...
	std::cout << "  P0: (" << r->GetP0()[0] << ", " << r->GetP0()[1] << ", " << r->GetP0()[2] << ")" << std::endl;
	std::cout << "  N1: (" << r->GetN1()[0] << ", " << r->GetN1()[1] << ", " << r->GetN1()[2] << ")" << std::endl;
	std::cout << "  N2: (" << r->GetN2()[0] << ", " << r->GetN2()[1] << ", " << r->GetN2()[2] << ")" << std::endl;
      }
	...
	std::cout << "  P0: (" << t->GetP0()[0] << ", " << t->GetP0()[1] << ", " << t->GetP0()[2] << ")" << std::endl;
	std::cout << "  N1: (" << t->GetN1()[0] << ", " << t->GetN1()[1] << ", " << t->GetN1()[2] << ")" << std::endl;
	std::cout << "  N2: (" << t->GetN2()[0] << ", " << t->GetN2()[1] << ", " << t->GetN2()[2] << ")" << std::endl;
      }

      if( KGMeshWire* w = dynamic_cast< KGMeshWire* >( *elementIt ) )
      {
	std::cout << "Element " << i << " is a wire." << std::endl;
	std::cout << "  P0: (" << w->GetP0()[0] << ", " << w->GetP0()[1] << ", " << w->GetP0()[2] << ")" << std::endl;
	std::cout << "  P1: (" << w->GetP1()[0] << ", " << w->GetP1()[1] << ", " << w->GetP1()[2] << ")" << std::endl;
