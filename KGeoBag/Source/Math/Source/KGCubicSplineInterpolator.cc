...
void KGCubicSplineInterpolator::Initialize(DataSet& data, double yp0, double ypn)
{
    ...
    for (unsigned int i = 1; i < n; i++) {
        ...
        fYp[i] = (sig - 1.) / p;
        u[i] = (fData[i + 1][1] - fData[i][1]) / (fData[i + 1][0] - fData[i][0]) -
               (fData[i][1] - fData[i - 1][1]) / (fData[i][0] - fData[i - 1][0]);
    ...
    if (ypn > 0.99e30)
        ...
        qn = 0.5;
        un = 3. / (fData[n][0] - fData[n - 1][0]) *
             (ypn - (fData[n][1] - fData[n - 1][1]) / (fData[n][0] - fData[n - 1][0]));
    }
