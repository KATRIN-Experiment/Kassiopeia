...
template<size_t NDIM> class KGBallSupportSet
{
  public:
    KGBallSupportSet()
    {
    ...
    KGBall<NDIM> GetMinimalBoundingBall() const
    {
        return fCurrentMinimalBoundingBall;
    };


    bool ComputeBoundingBall(const KGPoint<NDIM>* new_point, const std::vector<KGPoint<NDIM>>* points,
                             std::bitset<sizeof(size_t) * CHAR_BIT> mask, KGBall<NDIM>& ball)
    {
    ...
    bool CheckValidity(const std::vector<KGPoint<NDIM>>* points, std::bitset<sizeof(size_t) * CHAR_BIT> mask,
