...
class KGSpaceTreeNavigator: public KGNodeActor< KGNode< ObjectTypeList > >
{
    public:
        ...
        std::vector< KGNode<ObjectTypeList>* >* GetNodeList(){return &fNodeList;};

        KGNode<ObjectTypeList>* GetLeafNode(){return fNodeList[0];};
                ...
                if(fCube)
                {
                    ...
                    if(fCube->PointIsInside(fPoint))
                    {
                        ...
                        while(fNodeStack.top()->HasChildren())
                        {
                            ...
                            tree_prop = KGObjectRetriever<ObjectTypeList, KGSpaceTreeProperties<SpatialNDIM> >::GetNodeObject(fTempNode);
                            ...
                            unsigned int child_index = KGArrayMath::OffsetFromRowMajorIndex<SpatialNDIM>(fDimSize, fDelIndex);
                    ...
                    {
                        KGout<<"KGSpaceTreeNavigator::ApplyAction(): Warning, point:"<<fPoint[0]<<", "<<fPoint[1]<<", "<<fPoint[2]<<" not found in root node."<<KGendl;
                    }
