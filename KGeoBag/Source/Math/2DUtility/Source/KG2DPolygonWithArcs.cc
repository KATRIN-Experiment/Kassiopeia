...
KG2DPolygonWithArcs::KG2DPolygonWithArcs(const KG2DPolygonWithArcs& copyObject):
KG2DArea(copyObject)
{
...
void
KG2DPolygonWithArcs::SetDescriptors(const std::vector< KGVertexSideDescriptor >* ordered_descriptors)
{
...
void
KG2DPolygonWithArcs::Initialize()
{
...
void
KG2DPolygonWithArcs::GetVertices(std::vector<KTwoVector>* vertices) const
{
    vertices->clear();
    if(fIsValid)
    {
...
void
KG2DPolygonWithArcs::GetSides( std::vector< KG2DShape* >* sides) const
{
    sides->clear();
    if(fIsValid)
    {
...
void
KG2DPolygonWithArcs::NearestDistance( const KTwoVector& aPoint, double& aDistance ) const
{
    double min, dist;
...
KTwoVector
KG2DPolygonWithArcs::Point( const KTwoVector& aPoint) const
{
  KTwoVector aNearest;
    double min2, dist2;
    KTwoVector near;
...
KTwoVector
KG2DPolygonWithArcs::Normal( const KTwoVector& aPoint) const
{
  KTwoVector aNormal;
...
void
KG2DPolygonWithArcs::NearestIntersection( const KTwoVector& aStart, const KTwoVector& anEnd, bool& aResult, KTwoVector& anIntersection ) const
{
    bool found;
    double min2, dist2;
    KTwoVector inter;
...
bool
KG2DPolygonWithArcs::IsInside(const KTwoVector& point) const
{
...
void
KG2DPolygonWithArcs::DetermineIfPolygonIsSimpleAndValid()
{
            ...
            for(int j=0; j < fNVertices; j++)
            {
                if(ArcUnderTest != fSides[j]) //don't test the arc against itself
                {
                    ...
                    if(aLine)
                    {
                        ...
                        ArcUnderTest->NearestIntersection(aLine->GetFirstPoint(),
                                                   aLine->GetSecondPoint(),
                                                   test, inter);
                        if(test)
                        {
...
void
KG2DPolygonWithArcs::DetermineInteriorSide()
{
    int i,j,k;
    KTwoVector first;
    KTwoVector second;
