...
double
KG2DLineSegment::NearestDistance(const KTwoVector& p0, const KTwoVector& p1, const KTwoVector& aPoint)
{
    ...
    if(dot > 1.){return (aPoint - p1).Magnitude();}
...
KTwoVector
KG2DLineSegment::NearestPoint(const KTwoVector& p0, const KTwoVector& p1, const KTwoVector& aPoint )
{
    KTwoVector diff = p1 - p0;
    double len = diff.Magnitude();
    KTwoVector aNearest;
    KTwoVector del = aPoint - p0;
    double dot = del*diff/(len*len);
    if(dot < 0. ){aNearest = p0; return aNearest;}
    if(dot > 1.){aNearest = p1; return aNearest;}
    aNearest = p0 + dot*diff;
    return aNearest;
}

KTwoVector
KG2DLineSegment::NearestNormal(const KTwoVector& p0, const KTwoVector& p1, const KTwoVector& /*aPoint*/ )
{
    KTwoVector diff = p1 - p0;
    diff = diff.Unit();
    //normal always points in same direction (depends of the order of the points fP1, fP2)
...
bool
KG2DLineSegment::NearestIntersection(const KTwoVector& p0, const KTwoVector& p1, const KTwoVector& aStart, const KTwoVector& anEnd, KTwoVector& anIntersection )
{
    //aStart is refered to as p3
    //anEnd is refered to as p4
