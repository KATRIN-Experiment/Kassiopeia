...
KG2DPolyLine::KG2DPolyLine() : fIsValid(false)
{
    ;
}
...
void KG2DPolyLine::SetVertices(const std::vector<std::vector<double>>* ordered_vertices)
{
    fIsValid = false;
    fVertices.clear();
    for (const auto& ordered_vertice : *ordered_vertices) {
...
void KG2DPolyLine::SetVertices(const std::vector<KTwoVector>* ordered_vertices)
{
    fIsValid = false;
    fVertices.clear();
    for (const auto& ordered_vertice : *ordered_vertices) {
...
void KG2DPolyLine::Initialize()
{
    if (fVertices.size() < 2) {
        //this is not a polyline, what were you thinking?
        std::cout << "KG2DPolyLine::Initialize() Warning! Polyline has been assigned less than two vertices."
                  << std::endl;
        fIsValid = false;
        return;
    }
...
void KG2DPolyLine::NearestDistance(const KTwoVector& aPoint, double& aDistance) const
{
    double min, dist;
...
KTwoVector KG2DPolyLine::Point(const KTwoVector& aPoint) const
{
    KTwoVector aNearest;
    double min2, dist2;
    KTwoVector near;
...
KTwoVector KG2DPolyLine::Normal(const KTwoVector& aPoint) const
{
    KTwoVector aNormal;
...
void KG2DPolyLine::NearestIntersection(const KTwoVector& aStart, const KTwoVector& anEnd, bool& aResult,
...
void KG2DPolyLine::DetermineIfPolyLineIsSimple()
{
