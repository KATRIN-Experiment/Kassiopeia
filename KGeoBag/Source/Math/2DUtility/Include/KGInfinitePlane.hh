  ...
  class KGInfinitePlane
{
    public:
        ...
        virtual void NearestIntersection( const KThreeVector& aStart, const KThreeVector& anEnd, bool& aResult, KThreeVector& anIntersection ) const;

        KTwoVector Project(const KThreeVector& aPoint) const;

        KThreeVector GetOrigin() const {return fOrigin;}
        KThreeVector GetXAxis() const {return fXAxis;};
        KThreeVector GetYAxis() const {return fYAxis;};
        KThreeVector GetZAxis() const {return fZAxis;};

        void SetOrigin(const KThreeVector& origin) {fOrigin = origin;}
        void SetXAxis(const KThreeVector x_axis) {fXAxis = x_axis;};
        void SetYAxis(const KThreeVector y_axis) {fYAxis = y_axis;};
        void SetZAxis(const KThreeVector z_axis) {fZAxis = z_axis;};


        //static utility function for point-normal defined planes
        static double NearestDistance(const KThreeVector& origin, const KThreeVector& unit_normal, const KThreeVector& aPoint);

        static KThreeVector NearestPoint(const KThreeVector& origin, const KThreeVector& unit_normal, const KThreeVector& aPoint);

        static bool NearestIntersection(const KThreeVector& origin, const KThreeVector& unit_normal, const KThreeVector& aStart, const KThreeVector& anEnd, KThreeVector& anIntersection, double& distance);
