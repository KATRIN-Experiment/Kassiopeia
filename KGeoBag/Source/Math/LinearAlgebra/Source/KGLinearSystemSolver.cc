...
void KGLinearSystemSolver::SetMatrix(const double* mx)  //expects row major ordering, and an array of size fDim*fDim
{
...
void KGLinearSystemSolver::SetMatrixElement(unsigned int row, unsigned int col, const double& val)
{
    kg_matrix_set(fA, row, col, val);
}

void KGLinearSystemSolver::SetBVector(const double* vec)
{
...
void KGLinearSystemSolver::SetBVectorElement(unsigned int index, const double& val)
{
    kg_vector_set(fB, index, val);
}

void KGLinearSystemSolver::Reset()
{
    for (unsigned int row = 0; row < fDim; row++) {
...
void KGLinearSystemSolver::Solve()
{
...
void KGLinearSystemSolver::GetXVector(double* vec) const
{
    for (unsigned int i = 0; i < fDim; i++) {
...
double KGLinearSystemSolver::GetXVectorElement(unsigned int i) const
{
