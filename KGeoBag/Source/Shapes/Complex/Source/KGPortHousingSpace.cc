...
template<> void KGWrappedSpace<KGPortHousing>::VolumeInitialize(BoundaryContainer& aBoundaryContainer) const
{
...

    {
        ...
        disk->SetP0(KThreeVector(fObject->GetAMain(0), fObject->GetAMain(1), fObject->GetAMain(2)));
        disk->SetNormal(-1. * n);
    ...
    {
        auto disk = std::make_shared<KGDisk>();
        disk->SetP0(KThreeVector(fObject->GetBMain(0), fObject->GetBMain(1), fObject->GetBMain(2)));
        disk->SetNormal(n);
        disk->SetRadius(fObject->GetRMain());
        aBoundaryContainer.push_back(disk);
    }
    ...
    for (int i = 0; i < (int) fObject->GetNPorts(); i++) {
        ...
        if (const auto* r = dynamic_cast<const KGPortHousing::RectangularPort*>(p)) {
            ...
            KThreeVector p0(r->GetASub(0) - .5 * a, r->GetASub(1) - .5 * b, r->GetASub(2));

            auto rect = std::make_shared<KGRectangle>(a, b, p0, n1, n2);
            aBoundaryContainer.push_back(rect);
        }
        else if (const auto* c = dynamic_cast<const KGPortHousing::CircularPort*>(p)) {
            auto disk = std::make_shared<KGDisk>();
            disk->SetP0(KThreeVector(c->GetASub(0), c->GetASub(1), c->GetASub(2)));
            disk->SetNormal(KThreeVector(-1. * c->GetNorm(0), -1. * c->GetNorm(1), -1. * c->GetNorm(2)));
