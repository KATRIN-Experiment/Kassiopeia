...
bool KGNavigableMeshProximityCheck::BallIntersectsCube(const KGBall<KGMESH_DIM>& ball, const KGCube<KGMESH_DIM>& cube)
{
    ...
    KGPoint<KGMESH_DIM> delta;
    delta[0] = len_over_two;
    delta[1] = len_over_two;
    delta[2] = len_over_two;
...
void KGNavigableMeshProximityCheck::ApplyAction(KGMeshNavigationNode* node)
{
    fSphereIntersectsMesh = false;
    fLeafNodes.clear();
    ...
    if (node != nullptr) {
        ...
        for (auto& node : fLeafNodes) {
            ...
            for (unsigned int j = 0; j < n_elem; j++) {
                double edist =
                    fContainer->GetElement(element_list->GetID(j))->GetMeshElement()->NearestDistance(fPoint);
                if (edist < fRadius) {
                    fSphereIntersectsMesh = true;
                    return;
                }
