...
void KGRodSurfaceMesher::VisitWrappedSurface(KGWrappedSurface<KGRod>* rodSurface)
{
    ...
    for (unsigned int i = 0; i < rod->GetNCoordinates() - 1; i++)
        total_length += sqrt((rod->GetCoordinate(i, 0) - rod->GetCoordinate(i + 1, 0)) *
                                 (rod->GetCoordinate(i, 0) - rod->GetCoordinate(i + 1, 0)) +
                             (rod->GetCoordinate(i, 1) - rod->GetCoordinate(i + 1, 1)) *
                                 (rod->GetCoordinate(i, 1) - rod->GetCoordinate(i + 1, 1)) +
                             (rod->GetCoordinate(i, 2) - rod->GetCoordinate(i + 1, 2)) *
                                 (rod->GetCoordinate(i, 2) - rod->GetCoordinate(i + 1, 2)));
    ...
    for (int j = 0; j < rod->GetNDiscRad(); j++) {
        ...
        KThreeVector axis(rod->GetCoordinate(1, 0) - rod->GetCoordinate(0, 0),
                          rod->GetCoordinate(1, 1) - rod->GetCoordinate(0, 1),
                          rod->GetCoordinate(1, 2) - rod->GetCoordinate(0, 2));
        ...
        for (unsigned int i = 0; i < rod->GetNCoordinates() - 1; i++) {
            ...
            len_i = sqrt((rod->GetCoordinate(i, 0) - rod->GetCoordinate(i + 1, 0)) *
                             (rod->GetCoordinate(i, 0) - rod->GetCoordinate(i + 1, 0)) +
                         (rod->GetCoordinate(i, 1) - rod->GetCoordinate(i + 1, 1)) *
                             (rod->GetCoordinate(i, 1) - rod->GetCoordinate(i + 1, 1)) +
                         (rod->GetCoordinate(i, 2) - rod->GetCoordinate(i + 1, 2)) *
...
void KGRodSurfaceMesher::AddTrapezoid(const double* P1, const double* P2, const double* P3, const double* P4,
                                      const int nDisc)
{
