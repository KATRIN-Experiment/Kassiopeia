...
void KGQuadraticWireGridMesher::VisitWrappedSurface(KGWrappedSurface<KGQuadraticWireGrid>* quadraticWireGridSurface)
{
    ...
    auto numberOfSegments = static_cast<unsigned int>(maxR / segmentLength);
    ...
    for (unsigned int nWire = 1; nWire <= nWires; nWire++) {
        ...
        auto numberOfSegments = static_cast<unsigned int>(0.5 * xLengthIt / segmentLength);
        ...
        for (unsigned int xIt = 0; xIt < numberOfSegments; xIt++) {
            ...
            endPointYPP.SetComponents(startPointYPP.GetX() + segmentLength, yIt, 0.);

            // draw wire at y>0, x<0 (iteration over segments)
            endPointYPN.SetComponents(startPointYPN.GetX() - segmentLength, yIt, 0.);

            // draw wire at y<0, x>0 (iteration over segments)
            endPointYNP.SetComponents(startPointYNP.GetX() + segmentLength, -yIt, 0.);

            // draw wire at y<0, x<0 (iteration over segments)
            endPointYNN.SetComponents(startPointYNN.GetX() - segmentLength, -yIt, 0.);

            // draw perpendicular wires
            endPointXPP.SetComponents(yIt, startPointXPP.GetY() + segmentLength, 0.);
            endPointXPN.SetComponents(yIt, startPointXPN.GetY() - segmentLength, 0.);
            endPointXNP.SetComponents(-yIt, startPointXNP.GetY() + segmentLength, 0.);
            endPointXNN.SetComponents(-yIt, startPointXNN.GetY() - segmentLength, 0.);

            // add elements if height > wirePitch/2 to avoid wires with length = 0
            if (height > (wirePitch / 2.)) {
                KGMeshWire singleWireYPP(startPointYPP, endPointYPP, wireDiameter);
                auto* wYPP = new KGMeshWire(singleWireYPP);
                AddElement(wYPP);
                startPointYPP = endPointYPP;

                KGMeshWire singleWireYPN(startPointYPN, endPointYPN, wireDiameter);
                auto* wYPN = new KGMeshWire(singleWireYPN);
                AddElement(wYPN);
                startPointYPN = endPointYPN;

                KGMeshWire singleWireYNP(startPointYNP, endPointYNP, wireDiameter);
                auto* wYNP = new KGMeshWire(singleWireYNP);
                AddElement(wYNP);
                startPointYNP = endPointYNP;

                KGMeshWire singleWireYNN(startPointYNN, endPointYNN, wireDiameter);
                auto* wYNN = new KGMeshWire(singleWireYNN);
                AddElement(wYNN);
                startPointYNN = endPointYNN;

                KGMeshWire singleWireXPP(startPointXPP, endPointXPP, wireDiameter);
                auto* wXPP = new KGMeshWire(singleWireXPP);
                AddElement(wXPP);
                startPointXPP = endPointXPP;

                KGMeshWire singleWireXPN(startPointXPN, endPointXPN, wireDiameter);
                auto* wXPN = new KGMeshWire(singleWireXPN);
                AddElement(wXPN);
                startPointXPN = endPointXPN;

                KGMeshWire singleWireXNP(startPointXNP, endPointXNP, wireDiameter);
                auto* wXNP = new KGMeshWire(singleWireXNP);
                AddElement(wXNP);
                startPointXNP = endPointXNP;

                KGMeshWire singleWireXNN(startPointXNN, endPointXNN, wireDiameter);
    ...
    if (OuterCircle) {
        ...
        for (unsigned int circleIt = 0; circleIt < circleSegmentsIt; circleIt++) {
