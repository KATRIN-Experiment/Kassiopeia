...
void KGVTKMeshIntersectionTester::Render()
{
    ...
    for (unsigned int tIndex = 0; tIndex < fSampleCount; tIndex++) {
        ...
        if (fIntersectionCalculator.HasIntersectionWithMesh()) {
            ...
            vIntersectionPointId =
                fPoints->InsertNextPoint(tIntersectionPoint.X(), tIntersectionPoint.Y(), tIntersectionPoint.Z());
            fColors->InsertNextTuple3(fIntersectedLineColor.GetRed(),
                                      fIntersectedLineColor.GetGreen(),
                                      fIntersectedLineColor.GetBlue());
...
void KGVTKMeshIntersectionTester::Write()
{
    if (fWriteEnabled == true) {
        ...
        vismsg(eNormal) << "vtk mesh intersection tester <" << GetName() << "> is writing <"
                        << fPolyData->GetNumberOfCells() << "> cells to file <" << tFileName << ">" << eom;
